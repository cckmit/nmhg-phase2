...
public class InstallBaseSyncRetryTask extends QuartzJobBean implements StatefulJob {
    ...
    protected void executeInternal(JobExecutionContext context)
        ...
        try {
            ...
            logger.info("Inside InstallBaseSyncRetryTask: Starting item sync process by thread " + Thread.currentThread().getId());
            Calendar calStartTime = Calendar.getInstance();
            long startTimeOfInstallBaseSyncTask = calStartTime.getTimeInMillis();
            logger.info("Start Time of InstallBaseSyncRetryTask " + startTimeOfInstallBaseSyncTask);
            List<Long> syncTrackerIdsList = syncTrackerService.getIdsForRetryProcessing(IntegrationConstants.INSTALLBASE, maxNoOfRetries);
            commonSyncProcessor.syncProcessor(IntegrationConstants.INSTALLBASE, syncTrackerIdsList, false);
            Calendar calEndTime = Calendar.getInstance();
            long endTimeOfstartTimeOfInstallBaseSyncTask = calEndTime.getTimeInMillis();
            logger.info("InstallBaseSyncRetryTask was executed in "
                    + (endTimeOfstartTimeOfInstallBaseSyncTask - startTimeOfInstallBaseSyncTask)
                    + "m sec");
        } catch (Exception e) {
            logger.error("Error while running Installbase Sync Retry Task !!", e);
        }
    }
