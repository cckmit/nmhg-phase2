...
import com.tavant.clubcar.mockwebmethods.transformers.ExtWarrantyPriceCheckRequestTransformer;

public class WSExtWarrantyPriceCheckServiceImpl implements WSPriceCheckService {

	private static final Logger log = Logger
			.getLogger(WSExtWarrantyPriceCheckServiceImpl.class);

	public String checkPrice(String bod) {
		log.info("Inside WSExtWarrantyPriceCheckServiceImpl, recieved BOD " + bod);
		ExtendedWarrantyPriceFetch requestDTO = new ExtWarrantyPriceCheckRequestTransformer()
				.transform(bod);
	...
	private ExtendedWarrantyPriceCheckResponse createResponse(
		...
		for (PriceCheckRequestLineItemTypeDTO priceCheckRequestLineItemTypeDTO : lineItems) {
			...
			amt.setBigDecimalValue(new BigDecimal(123));
