package tavant.twms.integration.server.quartz.mbean;
...
@ManagedResource(
		objectName="bean:name=integrationServerQartzMonitor", description="Quartz Monitor of Integration Server"
)
public class QuartzMonitor implements JobListener{
	...
	public String getAllStatus() throws SchedulerException{
		errorIfNotAllowedOperation();
		String format = "|%1$-50s|%2$-20s|%3$-25s|%4$-25s|%5$-25s|\n";
		StringBuilder sb = new StringBuilder("<pre>");
		sb.append(String.format(format,"JobName","Status","Started On","Completed On","Next Fire Time"));
		...
		for(String jobName:jobNames){
			...
			sb.append(String.format(format,jobName,getStatus(triggers[0]),getLastStartTime(jobName),getLastCompletionTime(jobName),format(triggers[0].getNextFireTime())));
		}
		sb.append("</pre>");
		return sb.toString();
	}
