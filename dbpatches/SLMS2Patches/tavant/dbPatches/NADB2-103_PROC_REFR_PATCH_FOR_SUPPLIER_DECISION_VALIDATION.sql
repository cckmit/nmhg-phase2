...
BEGIN
  ...
  FOR each_rec IN all_rec LOOP
      ...
      END IF;
      IF each_rec.part_return_request IS NULL THEN
         v_error_code := common_utils.addErrorMessage(v_error_code, 'RC014');
      END IF;
       IF each_rec.part_return_request IS NOT NULL AND each_rec.return_location_code is NULL THEN
     ...
     IF(trim(lower(each_rec.decision))= 'accept')
        THEN 
           ...
           END;
           BEGIN
           IF each_rec.claim_amount_being_accepted IS NULL THEN
