...
BEGIN
  ...
  FOR each_rec IN all_rec LOOP
      ...
      END IF;
     IF each_rec.decision IS NOT NULL AND (trim(lower(each_rec.decision))= 'accepted') AND (each_rec.credit_memo_date IS NOT NULL OR each_rec.credit_memo_number IS NOT NULL OR each_rec.credit_memo_amount IS NOT NULL OR each_rec.credit_memo_currency IS NOT NULL) THEN
