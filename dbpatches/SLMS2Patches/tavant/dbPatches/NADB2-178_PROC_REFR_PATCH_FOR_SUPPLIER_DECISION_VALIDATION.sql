...
BEGIN
  ...
  FOR each_rec IN all_rec LOOP
      ...
      END IF;
     IF each_rec.decision IS NOT NULL AND (trim(lower(each_rec.decision))= 'accepted') AND each_rec.credit_memo_date is NOT NULL AND each_rec.credit_memo_number is NULL THEN
         v_error_code := common_utils.addErrorMessage(v_error_code, 'RC023');     
      END IF; 
     IF each_rec.decision IS NOT NULL AND (trim(lower(each_rec.decision))= 'accepted') AND each_rec.credit_memo_number is NOT NULL AND each_rec.credit_memo_date is NULL THEN
