...
BEGIN
  ...
  FOR each_rec IN all_rec LOOP
      ...
      END IF;
      IF(trim(lower(each_rec.decision))= 'part return request') AND (each_rec.part_return_request is NULL OR (trim(lower(each_rec.part_return_request))!='y')) THEN
         v_error_code := common_utils.addErrorMessage(v_error_code, 'RC014');
      END IF;
       IF each_rec.part_return_request IS NOT NULL AND (trim(lower(each_rec.part_return_request))='y') AND each_rec.return_location_code is NULL THEN
