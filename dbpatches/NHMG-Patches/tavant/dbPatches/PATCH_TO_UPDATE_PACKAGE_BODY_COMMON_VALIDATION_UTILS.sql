--PURPOSE    : PATCH TO UPDATE PACKAGE BODY COMMON_VALIDATION_UTILS
...
BEGIN
    ...
    IF UPPER(p_type) IN ('MACHINE SERIALIZED','ATTACHMENT SERIALIZED','PARTS WITH HOST','FPI') THEN
        RETURN TRUE;
...
Begin
    SELECT distinct i.item_number INTO v_item_number
      ...
      WHERE
        ...
        AND i.id=bi.item
        AND i.owned_by = 1 AND i.model = ig.ID 
        --AND i.service_part = 1                          --TODO: temporary fix by NMHG
...
IF v_type = 'STOCK' THEN
	...
	END IF;
END IF;
IF (upper(v_item_type) = 'MACHINE' AND upper(p_claim_type) NOT IN ('MACHINE SERIALIZED','MACHINE NON SERIALIZED','PARTS WITH HOST')) THEN   --TODO: temporary fix by NMHG for parts claim
