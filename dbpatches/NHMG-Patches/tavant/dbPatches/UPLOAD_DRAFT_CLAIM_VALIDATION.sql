...
SELECT truck_serial_number,campaign_code,COUNT(*)
	...
	v_COMPETITOR_MODEL_SERIAL_NUM VARCHAR2(2000) := NULL;
	v_BRAND_ON_COMPETITOR_MODEL VARCHAR2(2000) := NULL;
	v_competitor_model VARCHAR2(2000) := NULL;
...
BEGIN
  ...
  LOOP
   ...
   /* v_competitor_model_id := NULL;*/
	...
	v_caused_by := NULL;
	v_failure_type := NULL;
	v_COMPETITOR_MODEL_SERIAL_NUM := NULL;
	v_BRAND_ON_COMPETITOR_MODEL := NULL;
	v_competitor_model := NULL;
  ...
  BEGIN
	...
	ELSIF v_serialized THEN
		...
		ELSIF each_rec.container_number IS NOT NULL THEN
			...
			END IF;
		ELSIF NOT v_part_installed OR v_part_installed_on_tktsa THEN
			v_error_code := common_utils.addErrorMessage(v_error_code, 'DC004');
		END IF;
		IF v_err IS NOT NULL THEN
			v_error_code := common_utils.addErrorMessage(v_error_code, 'DC_'||v_err);
	...
	ELSIF NOT v_serialized THEN
		IF v_part_installed AND NOT v_part_installed_on_tktsa THEN
			IF (each_rec.competitor_model IS NULL OR each_rec.competitor_model = '') THEN
				v_error_code := common_utils.addErrorMessage(v_error_code, 'DC106');
			END IF;
			IF (each_rec.BRAND_ON_COMPETITOR_MODEL IS NULL OR each_rec.BRAND_ON_COMPETITOR_MODEL = '') THEN
				v_error_code := common_utils.addErrorMessage(v_error_code, 'DC106');
			END IF;
			IF (each_rec.COMPETITOR_MODEL_SERIAL_NUMBER IS NULL OR each_rec.COMPETITOR_MODEL_SERIAL_NUMBER = '') THEN
				v_error_code := common_utils.addErrorMessage(v_error_code, 'DC106');
			END IF;
			/*IF (each_rec.competitor_model_brand IS NULL OR each_rec.competitor_model_brand = '') THEN
				v_error_code := common_utils.addErrorMessage(v_error_code, 'DC106');
			/*ELSE
        ...
        ELSE
            ...
            FOR i IN 1 .. v_count LOOP
                ...
                IF UPPER(each_rec.claim_type) IN ('PART INSTALLED ON NON-SERIALIZED HOST TRUCK') THEN
                            IF UPPER(each_rec.PART_SERIAL_NUMBER) IS NULL
									AND UPPER(each_rec.Part_item_Number) IS NOT NULL THEN
                        ...
                        IF UPPER(each_rec.claim_type) IN ('PART INSTALLED ON SERIALIZED HOST TRUCK') THEN
                            ...
                            ELSIF UPPER(each_rec.Part_item_Number) IS NOT NULL THEN
                                      IF each_rec.Is_Serialized in ('Y') THEN    
                                            IF each_rec.Is_Part_Installed_on_OEM not in ('Y') THEN
                            ...
                            END IF;   
                         END IF; 
                        IF NOT common_validation_utils.isValidReplacedIRPart(null,
  ...
  IF v_error_code IS NULL THEN
	   ...
	   SET
		  ERROR_STATUS = 'Y',
			...
			/*competitor_model = v_competitor_model,*/
			/*competitor_model_brand = v_competitor_model_brand,*/
			/*competi_model_serial_num = v_competi_model_serial_num,*/
