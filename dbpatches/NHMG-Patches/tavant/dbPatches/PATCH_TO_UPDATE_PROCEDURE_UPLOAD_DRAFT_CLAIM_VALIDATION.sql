--PURPOSE    : PATCH TO UPDATE PROC UPLOAD_DRAFT_CLAIM_VALIDATION
--AUTHOR     : ROHIT MEHROTRA
--CREATED ON : 15-MAY-13
--IMPACT	 : DRAFT CLAIM UPLOAD
...
BEGIN
  ...
  IF v_repairenddate IS NOT NULL AND v_repairstartdate IS NOT NULL THEN 
      ...
      END IF;
  END IF;
  IF UPPER(EACH_REC.CLAIM_TYPE) IN ('PARTS WITHOUT HOST', 'PARTS WITH HOST')
  THEN
  ...
  END IF;
	IF UPPER(EACH_REC.CLAIM_TYPE) NOT IN ('FPI') THEN
		...
		END IF;
	END IF;
	IF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE NON SERIALIZED', 'PARTS WITH HOST', 'PARTS WITHOUT HOST') THEN
       ...
       IF EACH_REC.ALARM_CODES IS NOT NULL THEN          
        ...
        END IF; 
         EXCEPTION
            ...
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC067_IAC');
     END;
  IF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE SERIALIZED', 'MACHINE NON SERIALIZED', 'PARTS WITH HOST', 'FPI')
  THEN
  ...
  END IF;
	...
	END;
	BEGIN
    IF UPPER(EACH_REC.CLAIM_TYPE) NOT IN ('PARTS WITHOUT HOST') THEN
    ...
    IF UPPER(each_rec.claim_type) NOT IN ('PARTS WITHOUT HOST') AND 
        ...
        ELSE
            v_count := Common_Utils.count_delimited_values(each_rec.removed_oem_parts, v_delimiter);
             ...
             END IF;
            IF v_flag = TRUE THEN
            IF UPPER(each_rec.claim_type) IN ('PARTS WITH HOST') AND common_validation_utils.hasDuplicateSerializedPart(each_rec.REPLACED_OEM_PARTS_SERIAL_NUM,each_rec.removed_oem_parts, v_delimiter) THEN
