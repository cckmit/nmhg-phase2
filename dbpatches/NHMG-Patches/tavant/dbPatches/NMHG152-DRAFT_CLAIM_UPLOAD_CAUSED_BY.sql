ALTER TABLE STG_DRAFT_CLAIM ADD CAUSED_BY_ID NUMBER
/
BEGIN
  create_upload_error('draftWarrantyClaims','en_US','CAUSED_BY','DC116','Caused By can be specified only if Fault Found is specified');
  create_upload_error('draftWarrantyClaims','en_US','CAUSED_BY','DC117','Caused By is not valid');
END;
/
update upload_mgt set columns_to_capture=60 where name_of_template='draftWarrantyClaims'
/
COMMIT
/
create or replace
...
RETURN NUMBER;
 ...
 FUNCTION isValidFaultCodeForModelId(p_product NUMBER, p_model NUMBER, p_fault_code VARCHAR2, p_business_unit_name VARCHAR2)
 RETURN BOOLEAN;

 FUNCTION isValidJobCodeForModelId(p_product NUMBER,  p_model NUMBER, p_job_code VARCHAR2, p_business_unit_name VARCHAR2)
 RETURN BOOLEAN;

 FUNCTION isValidFaultFoundForModelId(p_product NUMBER, p_model NUMBER, p_fault_found VARCHAR2, p_business_unit_name VARCHAR2)
 RETURN BOOLEAN;
...
RETURN BOOLEAN;
 ...
 FUNCTION isValidSerialNumber(p_serial_number VARCHAR2,
	...
	p_dealer_id NUMBER, p_claim_type VARCHAR2,
...
FUNCTION isValidBrand(
  p_replaced_part VARCHAR2, 
  p_serial_number VARCHAR2,
  P_bu VARCHAR2)
RETURN varchar2;

FUNCTION isValidPartBrand(
  p_part_number VARCHAR2, 
  p_dealer_id VARCHAR2,
  P_bu VARCHAR2)
RETURN VARCHAR2;

FUNCTION getValidCausedBy(
	p_fault_found NUMBER, 
	p_caused_by VARCHAR2, 
	p_business_unit_name VARCHAR2)
RETURN NUMBER;
...
EXCEPTION 
    WHEN OTHERS THEN
    BEGIN                                                                  --TODO: temporary fix by NMHG
        SELECT fcd.code 
        INTO v_fault_code
        FROM fault_code_definition fcd, fault_code fc,
            (select fault_code from assembly where fault_code is not null 
                ...
                from failure_structure fs, failure_structure_assemblies fsa, assembly a
                where a.active = 1 and
                    fs.for_item_group = p_product and
                    fs.id = fsa.failure_structure and
                    fsa.assemblies = a.id )
            ) adata
        WHERE
...
EXCEPTION
    ...
    BEGIN                                                                 --TODO: temporary fix by NMHG
        ...
        where 
            ft.definition_id = ftd.failure_type_definition
            AND lower(ftd.name) = lower(ltrim(rtrim(p_fault_found))) 
            AND ft.for_item_group_id = p_product
            AND ft.d_active = 1
            AND ROWNUM = 1;
        RETURN TRUE;
    EXCEPTION
...
IS
  v_id         NUMBER := NULL;
BEGIN
    SELECT DISTINCT i.id INTO v_id
    FROM item i, item_group ig,inventory_item ii,brand_item bi,
        config_param p,config_value v,config_param_option o
    WHERE p.name='replacedItemsOnClaimConfiguration' AND
        v.config_param=p.id AND v.config_param_option=o.id AND
        UPPER(ig.name)=UPPER(o.value) AND
        ...
        i.owned_by in (SELECT org.id FROM party pty,organization org 
            WHERE pty.id=org.id AND pty.name='OEM') AND
        (bi.item_number=p_item_number and bi.item=i.id)
        AND ROWNUM=1;
...
BEGIN
    ...
    ELSIF UPPER(p_type) IN ('ATTACHMENT SERIALIZED','ATTACHMENT NON SERIALIZED') THEN
        v_type := 'Attachment';    
    ELSIF UPPER(p_type) IN ('PART INSTALLED ON SERIALIZED HOST TRUCK','PART INSTALLED ON NON-SERIALIZED HOST TRUCK') THEN
        v_type := 'Parts';
    ELSE
        v_type := 'Campaign';
    END IF;
    ...
    IF UPPER(p_type) IN ('MACHINE SERIALIZED','ATTACHMENT SERIALIZED','PART INSTALLED ON SERIALIZED HOST TRUCK','FPI') THEN
        RETURN TRUE;
    ELSIF UPPER(p_type) IN ('MACHINE NON SERIALIZED','ATTACHMENT NON SERIALIZED','PART INSTALLED ON NON-SERIALIZED HOST TRUCK') THEN
...
SELECT  DISTINCT i.item_number 
    INTO v_item_number
    FROM ITEM i, PARTY p, item_group ig, item_group pg,brand_item bi
    WHERE 
    UPPER(bi.item_number) = UPPER(ltrim(rtrim(p_item_number))) 
    AND I.ID=bi.ITEM
...
IS
  v_item_number         VARCHAR2(255) := NULL;
  v_item                VARCHAR2(255) := NULL;
BEGIN
    SELECT distinct i.item_number
    INTO v_item_number
    FROM item i,brand_item bi,
      item_group ig,
      item_group pg,
      config_param p,
      config_value v,
    ...
    AND bi.item_number        =p_item_number
    AND I.ID=bi.item
...
IS
  v_item_number         VARCHAR2(255) := NULL;
Begin
    SELECT distinct i.item_number INTO v_item_number
      FROM item i, item_group ig, item_group pg ,brand_item bi
      WHERE
        I.Business_Unit_Info = P_Business_Unit And I.D_Active = 1 
        AND bi.item_number =p_item_number 
        AND i.id=bi.item
...
IF v_type = 'STOCK' THEN
	...
	END IF;
END IF;
IF (upper(v_item_type) = 'MACHINE' AND upper(p_claim_type) NOT IN ('MACHINE SERIALIZED','MACHINE NON SERIALIZED','PART INSTALLED ON SERIALIZED HOST TRUCK')) THEN   --TODO: temporary fix by NMHG for parts claim
...
BEGIN
	p_error_code := NULL;
	SELECT DISTINCT i.id INTO v_item
	FROM item i, item_group ig, item_group pg,brand_item bi,
	config_param cp, config_value cv, config_param_option po
	WHERE UPPER(bi.item_number)=UPPER(p_item_number)
    AND i.id=bi.item
		AND i.d_active=1 AND i.business_unit_info=p_bu
    ...
    AND pg.group_code='PARTS';          --TODO: temporary fix by NMHG
	RETURN v_item;
EXCEPTION
	WHEN NO_DATA_FOUND THEN
...
RETURN VARCHAR2
IS
  v_brand VARCHAR2(255);
BEGIN
  SELECT distinct bi.brand INTO v_brand from brand_item bi, item i                 
  WHERE bi.item=i.id
  AND bi.item_number = p_replaced_part 
  AND i.business_unit_info= p_bu 
  AND upper(i.status) = 'ACTIVE'
  AND bi.brand 
  IN (SELECT ii.brand_type FROM inventory_item ii WHERE ii.serial_number=p_serial_number AND ii.business_unit_info=p_bu);
  RETURN v_brand;
EXCEPTION
  WHEN NO_DATA_FOUND THEN
		RETURN NULL;
...
IS
  v_brand VARCHAR2(255);
BEGIN
 SELECT bi.brand INTO v_brand
  FROM brand_item bi, item i, dealership d
  WHERE bi.item_number = p_part_number
  AND bi.item = i.id
  AND i.business_unit_info=p_bu
  AND d.id = p_dealer_id
  AND d.brand = bi.brand
  AND i.item_type like 'PART%';
  RETURN v_brand;
EXCEPTION
  WHEN NO_DATA_FOUND THEN
		RETURN NULL;
END isValidPartBrand;

FUNCTION getValidCausedBy(p_fault_found NUMBER, p_caused_by VARCHAR2, p_business_unit_name VARCHAR2)
RETURN NUMBER
IS
	v_fcd number;
BEGIN
    select fcd.id into v_fcd
	from
		failure_cause fc,
		failure_cause_definition fcd,
		i18nfailure_cause_definition ifcd
		where fc.failure_type_id=p_fault_found
		and fc.definition_id = fcd.id
		and ifcd.failure_cause_definition=fcd.id
		and upper(ifcd.name)=upper(trim(p_caused_by))
		and rownum=1;
	RETURN v_fcd;
EXCEPTION 
    WHEN OTHERS THEN
        RETURN NULL;
END getValidCausedBy;
...
CURSOR ALL_REC IS
	...
	WHERE NVL(ERROR_STATUS,'N') = 'N' -- AND
		 ORDER BY ID ASC;
CURSOR DUP_REC IS
SELECT truck_serial_number,campaign_code,COUNT(*)
          FROM STG_DRAFT_CLAIM
          WHERE claim_type='FPI'
          GROUP BY truck_serial_number,campaign_code
          HAVING COUNT(*)>1;  
    ...
    v_part NUMBER;
    v_model_id NUMBER;
    v_repairstartdate   DATE := NULL;
    v_brand  VARCHAR2(255);
    v_replaced_part_num     VARCHAR2(255) := NULL;
	v_caused_by NUMBER;
	v_failure_type NUMBER;
BEGIN
    BEGIN
     FOR EACH_DUP_REC IN DUP_REC
      LOOP
        ...
        WHERE truck_serial_number = EACH_DUP_REC.truck_serial_number
    ...
    WHERE u.id = f.uploaded_by 
        and orgu.org_user=u.id and orgu.belongs_to_organizations=o.id AND f.id=(SELECT file_upload_mgt_id FROM stg_draft_claim WHERE rownum = 1) and rownum = 1;
  ...
  LOOP
    ...
    v_job_code := NULL;
    v_fault_found := NULL;
    v_truck_serial_number := ltrim(rtrim(each_rec.truck_serial_number));
    v_err := NULL;
    v_inv := NULL;
    v_item := NULL;
    v_part_serial := NULL;
    ...
    v_repairenddate := NULL;
    v_failuredate := NULL;
    v_installeddate := NULL;
    v_repairstartdate := NULL;
    v_brand := NULL;
    v_replaced_part_num   := NULL;
	v_caused_by := NULL;
	v_failure_type := NULL;
	IF EACH_REC.REPAIR_END_DATE IS NOT NULL AND 
		COMMON_VALIDATION_UTILS.isValidDate(each_rec.REPAIR_END_DATE, 'YYYY-MM-DD') THEN
		v_repairenddate := TO_DATE(each_rec.repair_end_date,'YYYY-MM-DD');
	ELSIF EACH_REC.REPAIR_END_DATE IS NOT NULL AND 
		COMMON_VALIDATION_UTILS.isValidDate(each_rec.REPAIR_END_DATE, 'YYYYMMDD') THEN
		v_repairenddate := TO_DATE(each_rec.repair_end_date,'YYYYMMDD');
	ELSE
		v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC018');
	END IF;   
  IF EACH_REC.REPAIR_START_DATE IS NOT NULL AND 
		COMMON_VALIDATION_UTILS.isValidDate(each_rec.REPAIR_START_DATE, 'YYYY-MM-DD') THEN
		v_repairstartdate := TO_DATE(each_rec.repair_start_date,'YYYY-MM-DD');
	ELSIF EACH_REC.REPAIR_START_DATE IS NOT NULL AND 
		COMMON_VALIDATION_UTILS.isValidDate(each_rec.REPAIR_START_DATE, 'YYYYMMDD') THEN
		v_repairstartdate := TO_DATE(each_rec.repair_start_date,'YYYYMMDD');
	ELSE
		v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC108');
	END IF;
  IF v_repairenddate IS NOT NULL AND v_repairstartdate IS NOT NULL THEN 
      IF NOT v_repairenddate >= v_repairstartdate THEN
        v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC109');
      END IF;
  END IF;
  IF UPPER(EACH_REC.CLAIM_TYPE) IN ('PART INSTALLED ON NON-SERIALIZED HOST TRUCK', 'PART INSTALLED ON SERIALIZED HOST TRUCK')
  THEN
    IF EACH_REC.HOURS_ON_TRUCK_DURING_INSTALL IS NULL THEN
      v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC110');
    END IF;

    IF EACH_REC.HOURS_ON_TRUCK_DURING_INSTALL IS NOT NULL AND (EACH_REC.HOURS_ON_TRUCK_DURING_INSTALL < 0 OR EACH_REC.HOURS_ON_TRUCK_DURING_INSTALL > 999999) THEN
      v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC111');
    END IF;
  END IF;
	IF UPPER(EACH_REC.CLAIM_TYPE) NOT IN ('FPI') THEN
		...
		END IF;
	END IF;
	IF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE NON SERIALIZED', 'PART INSTALLED ON SERIALIZED HOST TRUCK', 'PART INSTALLED ON NON-SERIALIZED HOST TRUCK') THEN
	...
	END IF;
  BEGIN
    IF UPPER(EACH_REC.CLAIM_TYPE) IN ('PART INSTALLED ON NON-SERIALIZED HOST TRUCK', 'PART INSTALLED ON SERIALIZED HOST TRUCK') THEN
      IF EACH_REC.BRAND IS NULL THEN
        v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC112');
      ELSE
        IF each_rec.PART_ITEM_NUMBER IS NOT NULL THEN
          v_brand := common_validation_utils.isValidPartBrand(each_rec.PART_ITEM_NUMBER,v_dealer_id,v_bu_name);
        ELSIF each_rec.PART_SERIAL_NUMBER IS NOT NULL THEN
          v_brand := common_validation_utils.isValidPartBrand(each_rec.PART_SERIAL_NUMBER,v_dealer_id,v_bu_name);   
        END IF; 
        IF v_brand IS NULL THEN
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC115');   
        ELSIF v_brand != each_rec.BRAND THEN
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC112');

        END IF;    
      END IF;    
    ELSIF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE SERIALIZED') THEN
      IF EACH_REC.BRAND IS NOT NULL THEN
       v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC112');
      ELSIF each_rec.REMOVED_OEM_PARTS is NOT NULL THEN
       v_count := Common_Utils.count_delimited_values_new(each_rec.REMOVED_OEM_PARTS, '#$#');           
       FOR i IN 1..v_count LOOP
         v_replaced_part_num := common_utils.get_delimited_value(each_rec.REMOVED_OEM_PARTS, '#$#', i);
         v_brand := common_validation_utils.isValidBrand(v_replaced_part_num,each_rec.TRUCK_SERIAL_NUMBER,v_bu_name);
         IF v_brand IS NULL THEN
           v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC114');   
         ELSE
            UPDATE stg_draft_claim SET brand = v_brand WHERE id=each_rec.id;
         END IF;
       END LOOP;  
      END IF;  
    ELSIF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE NON SERIALIZED') THEN
      IF EACH_REC.BRAND IS NOT NULL THEN
       v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC112');
      ELSE 
       v_count := Common_Utils.count_delimited_values_new(each_rec.REMOVED_OEM_PARTS, '#$#');           
       FOR i IN 1..v_count LOOP
         v_replaced_part_num := common_utils.get_delimited_value(each_rec.REMOVED_OEM_PARTS, '#$#', i);
         v_brand := common_validation_utils.isValidPartBrand(v_replaced_part_num,v_dealer_id,v_bu_name);
         IF v_brand IS NULL THEN
           v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC114');   
         ELSE
            UPDATE stg_draft_claim SET brand = v_brand WHERE id=each_rec.id;
    ...
    END IF;   
	END;
  IF EACH_REC.AUTHORIZATION_RECEIVED IS NOT NULL AND EACH_REC.AUTHORIZATION_RECEIVED = 'Y' THEN
          IF EACH_REC.AUTHORIZATION_NUMBER IS NULL THEN
              v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC113');
          END IF;
	END IF;
  BEGIN
	...
	BEGIN
        IF each_rec.claim_type IS NULL OR UPPER(each_rec.claim_type) NOT IN 
            ('MACHINE SERIALIZED', 'MACHINE NON SERIALIZED', 'PART INSTALLED ON SERIALIZED HOST TRUCK', 
            'PART INSTALLED ON NON-SERIALIZED HOST TRUCK', 'FPI')
        THEN
        ...
        END IF;
	END;
    IF UPPER(each_rec.claim_type) IN ('FPI') THEN
        IF each_rec.campaign_code IS NULL THEN
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC026');
        ELSIF NOT common_validation_utils.isValidCampaignCode(each_rec.campaign_code, v_service_provider, v_bu_name,v_repairenddate,v_service_provider_number,v_truck_serial_number) THEN
	...
	END IF;
	IF UPPER(each_rec.claim_type) = 'PART INSTALLED ON NON-SERIALIZED HOST TRUCK' THEN
		v_serialized := NULL;
		v_part_installed_on_tktsa := NULL;
	ELSIF UPPER(each_rec.claim_type) in ('FPI',
		'MACHINE SERIALIZED','MACHINE NON SERIALIZED') THEN
		v_part_installed := NULL;
		v_part_installed_on_tktsa := NULL;
	ELSIF UPPER(each_rec.claim_type) = 'PART INSTALLED ON SERIALIZED HOST TRUCK' AND v_serialized THEN
		v_part_installed_on_tktsa := NULL;
	END IF;
	IF NOT v_serialized AND UPPER(each_rec.claim_type) IN 
			('MACHINE SERIALIZED','FPI') THEN
	...
	END IF;
	IF UPPER(each_rec.claim_type) = 'PART INSTALLED ON SERIALIZED HOST TRUCK' 
			AND NOT v_part_installed THEN
		v_error_code := common_utils.addErrorMessage(v_error_code, 'DC102');
	END IF;
	IF UPPER(each_rec.claim_type) = 'PART INSTALLED ON NON-SERIALIZED HOST TRUCK' 
	...
	IF v_machine_serial_number IS NOT NULL THEN
		...
		ELSIF v_truck_serial_number IS NULL THEN
			v_error_code := common_utils.addErrorMessage(v_error_code, 'DC004');
		ELSIF UPPER(v_truck_serial_number) != UPPER(v_machine_serial_number) THEN
			v_error_code := common_utils.addErrorMessage(v_error_code, 'DC_90');
		END IF;
	ELSIF v_serialized THEN
		IF v_truck_serial_number IS NOT NULL THEN
			IF UPPER(each_rec.claim_type)='FPI' THEN
				v_inv := common_validation_utils.isValidInventoryForFieldMod(
							v_truck_serial_number,each_rec.campaign_code,
							v_service_provider,v_bu_name,v_repairenddate,v_err);
			ELSE
				v_inv := common_validation_utils.isValidSerialNumber(
								v_truck_serial_number, v_bu_name, each_rec.model_number,
       ...
       IF EACH_REC.ALARM_CODES IS NOT NULL THEN          
        ...
        END IF; 
         EXCEPTION
            ...
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC067_IAC');
     END;
  IF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE SERIALIZED', 'MACHINE NON SERIALIZED', 'PART INSTALLED ON SERIALIZED HOST TRUCK', 'FPI')
  THEN
    IF EACH_REC.HOURS_ON_TRUCK IS NULL
    THEN
      v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC016');
    END IF;
    IF EACH_REC.HOURS_ON_TRUCK IS NOT NULL AND (EACH_REC.HOURS_ON_TRUCK < 0 OR EACH_REC.HOURS_ON_TRUCK > 999999)
    THEN
  ...
  END IF;
	...
	END;
	BEGIN
    IF UPPER(EACH_REC.CLAIM_TYPE) NOT IN ('PART INSTALLED ON NON-SERIALIZED HOST TRUCK') THEN
    ...
    END IF;
	END;
    IF UPPER(each_rec.claim_type) NOT IN ('PART INSTALLED ON NON-SERIALIZED HOST TRUCK') AND 
            each_rec.removed_oem_parts IS NOT NULL THEN

         IF  each_rec.INSTALLED_OEM_PARTS IS NULL THEN            
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC045_ADD_INP');         
         END IF;    
        v_flag := TRUE;
        IF NOT common_validation_utils.isValidDelimitedValue(each_rec.removed_oem_parts, v_delimiter) THEN
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC045');
            v_flag := FALSE;
        ELSE
            v_count := Common_Utils.count_delimited_values(each_rec.removed_oem_parts, v_delimiter);
             ...
             END IF;
            IF v_flag = TRUE THEN
            IF UPPER(each_rec.claim_type) IN ('PART INSTALLED ON SERIALIZED HOST TRUCK') AND common_validation_utils.hasDuplicateSerializedPart(each_rec.REPLACED_OEM_PARTS_SERIAL_NUM,each_rec.removed_oem_parts, v_delimiter) THEN
                  v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_98');          
            END IF;
            FOR i IN 1 .. v_count LOOP
                IF i <= v_count2 THEN
                IF UPPER(each_rec.claim_type) IN ('PART INSTALLED ON NON-SERIALIZED HOST TRUCK') THEN
                            IF UPPER(each_rec.PART_SERIAL_NUMBER) IS NOT NULL THEN
                               ...
                               ELSE
                                                     ...
                                                     ELSIF common_utils.get_delimited_value(each_rec.removed_oem_parts_quantity, v_delimiter, i) !=1 THEN
                            ...
                            ELSIF UPPER(each_rec.Part_item_Number) IS NOT NULL THEN
                                 IF each_rec.Removed_OEM_Parts IS NOT NULL THEN
                                                  IF common_utils.get_delimited_value(each_rec.Removed_OEM_Parts, v_delimiter, i) != each_rec.Part_item_Number THEN
                                                          v_error_code := Common_Utils.addErrorMessage(v_error_code, 'Please add correct replaced part');
                                                   END IF;
                                  END IF;                         
                                  IF UPPER(each_rec.REMOVED_OEM_PARTS_QUANTITY) IS NOT NULL THEN
                                                  IF common_utils.get_delimited_value(each_rec.removed_oem_parts_quantity, v_delimiter, i) !=1 THEN
                                                          v_error_code := Common_Utils.addErrorMessage(v_error_code, 'Replaced part quantity should be one only');   
                                                   END IF;
                                  END IF;                 
                             END IF;
                END IF; 
                        IF UPPER(each_rec.claim_type) IN ('PART INSTALLED ON SERIALIZED HOST TRUCK') THEN
                                      ...
                                      IF each_rec.Is_Serialized in ('Y') THEN    
                                            ...
                                            ELSE
                                                  IF each_rec.REPLACED_OEM_PARTS_SERIAL_NUM IS NOT NULL THEN
                                                          v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_82'); 
                                                  END IF;
                                                  IF each_rec.Removed_OEM_Parts IS NOT NULL AND UPPER(each_rec.Part_item_Number) != common_utils.get_delimited_value(each_rec.Removed_OEM_Parts, v_delimiter, 1) THEN
                                                          v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_85');
                                                   END IF;
                                                   IF Common_Utils.count_delimited_values(each_rec.Removed_OEM_Parts, v_delimiter)  !=1 THEN
                                       ...
                                       ELSE
                                              IF each_rec.REPLACED_OEM_PARTS_SERIAL_NUM IS NOT NULL THEN
                                               ...
                                               END IF;
                                              IF each_rec.Removed_OEM_Parts IS NOT NULL AND UPPER(each_rec.Part_item_Number) != common_utils.get_delimited_value(each_rec.Removed_OEM_Parts, v_delimiter, i) THEN
                                                          v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_85');
                                              END IF;
                                              IF Common_Utils.count_delimited_values(each_rec.Removed_OEM_Parts, v_delimiter)  !=1 THEN
                                                    v_error_code := Common_Utils.addErrorMessage(v_error_code, 'Should not add more than one part');
                        ...
                        IF NOT common_validation_utils.isValidReplacedIRPart(common_utils.get_delimited_value(each_rec.REPLACED_OEM_PARTS_SERIAL_NUM, v_delimiter, i),
                              common_utils.get_delimited_value(each_rec.removed_OEM_parts, v_delimiter, i), v_bu_name)
                        THEN
                ...
                END IF;     
            END LOOP;
			IF v_replaced_parts IS NOT NULL AND v_replaced_parts != each_rec.removed_OEM_parts THEN
                UPDATE stg_draft_claim SET removed_OEM_parts=v_replaced_parts WHERE id=each_rec.id;
            END IF;
            END IF;   
        END IF;
        IF each_rec.removed_oem_parts_quantity IS NULL THEN
			 v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC027');
        ELSIF NOT common_validation_utils.isValidDelimitedValue(each_rec.removed_oem_parts_quantity, v_delimiter) THEN
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC047');
        ELSIF v_flag = TRUE AND v_count != Common_Utils.count_delimited_values(each_rec.removed_oem_parts_quantity, v_delimiter) THEN
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC048');
        ELSIF v_flag = TRUE THEN
            FOR i IN 1 .. v_count LOOP
                IF NOT common_utils.isPositiveInteger(
                        common_utils.get_delimited_value(each_rec.removed_oem_parts_quantity, v_delimiter, i)) 
                THEN
	...
	IF (UPPER(each_rec.claim_type) = 'MACHINE NON SERIALIZED' OR
			(UPPER(each_rec.claim_type) = 'PART INSTALLED ON SERIALIZED HOST TRUCK' 
...
END;
BEGIN
       IF UPPER(EACH_REC.REMOVED_OEM_PARTS)IS NOT NULL THEN 
...
END;
END IF;
	IF v_repairenddate IS NOT NULL AND v_failuredate IS NOT NULL 
			AND v_repairenddate < v_failuredate THEN
	...
	BEGIN
        v_fault_code := common_utils.getValidFaultCode(EACH_REC.fault_location);
		 IF v_product IS NOT NULL AND v_model IS NOT NULL AND v_fault_code IS NOT NULL AND 
       NOT COMMON_VALIDATION_UTILS.isValidFaultCodeForModelId(v_product, v_model, v_fault_code, v_bu_name)                   --TODO: temporary fix by NMHG
		 THEN
	...
	IF each_rec.job_code IS NOT NULL THEN
        ...
        FOR i IN 1 .. v_count LOOP
            ...
            IF v_job_code IS NULL THEN
                ...
                EXIT;
            ELSIF v_product IS NOT NULL AND v_model IS NOT NULL AND
                NOT common_validation_utils.isValidJobCodeForModelId(v_product, v_model, v_job_code, v_bu_name)               --TODO: temporary fix by NMHG
		    THEN
			    v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC039');
                EXIT;
            ELSE
	...
	BEGIN
		 IF v_model IS NOT NULL  AND EACH_REC.FAULT_FOUND IS NOT NULL THEN
            IF NOT COMMON_VALIDATION_UTILS.isValidFaultFoundForModelId(v_product, v_model, EACH_REC.FAULT_FOUND, v_bu_name)
            THEN
              v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC040');
            ELSE
              BEGIN
                v_valid_fault_found := TRUE;
                SELECT ftd.name,ft.id INTO v_fault_found,v_failure_type
                FROM failure_type ft, 
                ...
                EXCEPTION
                    ...
                    where 
                        ft.definition_id = i18n_ftd.failure_type_definition
                        AND lower(i18n_ftd.name) = lower(ltrim(rtrim(each_rec.fault_found))) 
                        AND ft.for_item_group_id = v_product 
                        AND ft.d_active = 1
                        AND ftd.id = i18n_ftd.failure_type_definition
                        AND ROWNUM = 1; 
              END ;
	...
	END;
	BEGIN
		IF EACH_REC.FAULT_FOUND IS NULL AND EACH_REC.caused_by IS NOT NULL THEN
			v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC116');
		ELSIF v_valid_fault_found AND EACH_REC.caused_by IS NOT NULL THEN
			v_caused_by := COMMON_VALIDATION_UTILS.getValidCausedBy(v_failure_type, EACH_REC.caused_by, v_bu_name);
			IF v_caused_by IS NULL THEN
				v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC117');
			END IF;
		END IF;
	END;
    BEGIN
  ...
  IF v_error_code IS NULL THEN
	   ...
	   SET
		  ERROR_STATUS = 'Y',
            ...
            repair_end_date= CAST (each_rec.REPAIR_END_DATE AS number(8,0)),               
            repair_start_date= CAST (each_rec.REPAIR_START_DATE AS number(8,0)),                      
