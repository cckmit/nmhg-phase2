...
AS
  ...
  v_is_update       BOOLEAN;
  v_user            NUMBER;
  v_org_addr_id     NUMBER;
  v_company_type    VARCHAR2(255);
  v_count_org_addr_id NUMBER;
  v_site_addr_id    NUMBER;
BEGIN
    ...
    BEGIN
      ...
      IF v_is_update THEN
        IF UPPER(EACH_REC.CUSTOMER_TYPE) IN ('DEALERSHIP','THIRD PARTY','NATIONAL ACCOUNT','ORIGINAL EQUIPMENT MANUFACTURE','DIRECT CUSTOMER','INTER COMPANY') THEN
          ...
          FROM SERVICE_PROVIDER SP,
            BU_ORG_MAPPING BOM,
            PARTY P
          WHERE UPPER(SP.SERVICE_PROVIDER_NUMBER) = UPPER(EACH_REC.CUSTOMER_NUMBER)
        ...
        SELECT count(ORG_ADDRESSES)
        INTO v_count_org_addr_id
        FROM ORGANIZATION_ORG_ADDRESSES
        WHERE organization             =v_party_id group by organization;
        
         IF Upper(Each_Rec.Dealer_Site)IS NOT NULL and  v_count_org_addr_id =1 THEN
		      SELECT ORG_ADDRESSES
           INTO v_org_addr_id
            FROM ORGANIZATION_ORG_ADDRESSES
            WHERE organization             =v_party_id ;
		      select abam.address_id 
          INTO v_site_addr_id 
          from address_book ab,address_book_address_mapping abam 
          where ab.belongs_to=v_party_id 
          and ab.d_active=1 and Upper(ab.type)='SELF' and abam.address_book_id=ab.id ;
		 
          UPDATE Organization_Address
          SET location = each_rec.addressline1
            ||COALESCE('-'
        ...
        UPDATE party
        SET Name                  = Each_Rec.Customer_Name,
          d_active                = v_d_active,
          D_Updated_On            = Sysdate,
          D_Updated_Time          = systimestamp,
          d_last_updated_by       = v_user,
        ...
        WHERE id                  = v_party_id;
        UPDATE organization
        SET preferred_currency = each_rec.currency
        WHERE id               = v_party_id;
      END IF;
      IF NOT v_is_update THEN
        ...
        IF UPPER
          ...
          THEN
		  SELECT DECODE( UPPER(EACH_REC.CUSTOMER_TYPE), 'DEALERSHIP','Dealer', 'THIRD PARTY','THIRD PARTY', 'NATIONAL ACCOUNT','NATIONAL ACCOUNT', 'ORIGINAL EQUIPMENT MANUFACTURE','OEM', 'DIRECT CUSTOMER','DIRECT CUSTOMER', 'INTER COMPANY', 'INTERCOMPANY')
          INTO v_company_type
          FROM dual;
          INSERT
          INTO SERVICE_PROVIDER
            ...
            (
              ...
              EACH_REC.CUSTOMER_NUMBER,
              UPPER(EACH_REC.STATUS),
              EACH_REC.DEALER_FAMILY,
			  v_company_type
            );
          IF UPPER
            (
              EACH_REC.BUSINESS_UNIT
            )
            = 'HUSSMANN' AND EACH_REC.CUSTOMER_TYPE = 'DEALERSHIP' THEN
        ...
        ELSIF UPPER
          ...
          = 'NATIONAL ACCOUNT' THEN
          INSERT
          INTO NATIONAL_ACCOUNT
            (
              ID,
        ...
        ELSIF UPPER
          ...
          = 'ORIGINAL EQUIPMENT MANUFACTURE' THEN
          INSERT
          INTO ORIGINAL_EQUIP_MANUFACTURER
            (
              ID,
        ...
        ELSIF UPPER
          ...
          = 'DIRECT CUSTOMER' THEN
          INSERT
          INTO DIRECT_CUSTOMER
            (
              ID,
