    ...
    BEGIN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN      
                ...
                SELECT FAULT_CODE_DEFN_SEQ.NEXTVAL-1
                INTO   v_fault_code_def_id
                FROM DUAL;
        ...
        BEGIN
			SELECT count(*) INTO v_count /*+INDEX(ASSEMBLY_ASSEMBLYDEFINITION_IX a) INDEX(ASSEMBLY_ISPARTOFASSEMBLY_IX a)*/
            FROM assembly a,
            ...
            ELSE
				...
				WHERE a.assembly_definition = v_system_code_id
					...
					AND fsa.failure_structure = fs.ID
					AND fs.for_item_group = v_item_group_id
					AND a.active=0 AND rownum=1;
			
                UPDATE assembly SET active = 1 WHERE ID = v_current_assembly;

                UPDATE assembly SET active=0 
                WHERE is_part_of_assembly=v_current_assembly and active=1;
                
                UPDATE action_node SET active = 0
                WHERE defined_for=v_current_assembly and active=1;
            END IF;
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                SELECT assembly_seq.NEXTVAL-1
                INTO   v_current_assembly
                FROM DUAL;
        ...
        BEGIN
            ...
            BEGIN
				...
				SELECT count(*)     /*+INDEX(ASSEMBLY_ASSEMBLYDEFINITION_IX ) INDEX(ASSEMBLY_ISPARTOFASSEMBLY_IX )*/
				INTO v_count
                FROM assembly
				...
				IF v_count > 0 THEN
					SELECT ID INTO v_current_assembly
					FROM assembly
					WHERE assembly_definition = v_code_id
						AND is_part_of_assembly = v_parent_assembly
        ...
        IF v_fault_code_id IS NULL THEN
                SELECT fault_code_seq.NEXTVAL-1 INTO v_fault_code_id FROM DUAL;
        ...
        BEGIN
			...
			SELECT count(*)       /*+INDEX(ACTIONNODE_DEFINEDFOR_IX) INDEX(ACTIONNODE_DEFINITION_IX)*/
            INTO   v_count
            FROM action_node
            WHERE defined_for = v_current_assembly
            AND definition = v_action_code_id and active=1;
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN   
                SELECT action_node_seq.NEXTVAL-1
                INTO   v_action_node
                FROM DUAL;
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN    
               SELECT SERVICE_PROCEDUREDEF_SEQ.NEXTVAL-1
               INTO   v_service_proc_def_id
               FROM DUAL;
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                ...
                SELECT service_procedure_seq.NEXTVAL-1
                INTO   v_service_proc_id
                FROM DUAL;
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                SELECT FAILURE_STRUCT_SEQ.NEXTVAL-1
                INTO   v_fail_struct_id
                FROM DUAL;
