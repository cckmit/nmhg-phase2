                    ...
                    END;
                              ...
                              IF EACH_REC.Is_Part_Installed IN ('Y') THEN
                                 ...
                                 ELSE
                                    ...
                                    END IF;
									IF EACH_REC.Model_Number IS NOT null THEN 
                                           v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_94');                                          
                                       
                                    END IF;
									IF EACH_REC.Competitor_Model IS NOT null THEN 
                                           v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_95');                                          
                                       
                                    END IF;
                                 END IF;
                               ELSE
                                      IF v_machine_serial_number IS NOT null THEN
                                         v_error_code := Common_Utils.addErrorMessage(v_error_code, 'This Part is already installed on Unit. Please file claim as Part installed on serialized host machine.');                              
                                      END IF;
                                      IF  EACH_REC.Model_Number IS NOT null THEN
                                           v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_94');
                                      ELSIF  EACH_REC.Competitor_Model IS NOT null THEN
        ...
        ELSE
            ...
            FOR i IN 1 .. v_count LOOP
                                      ...
                                      IF each_rec.Is_Serialized in ('Y') THEN    
                                       ...
                                       ELSE
                                              ...
                                              IF each_rec.REPLACED_IR_PARTS_SERIAL_NUM IS NOT NULL THEN
                                               ...
                                               IF each_rec.replaced_ir_parts_quantity IS NOT NULL AND common_utils.get_delimited_value(each_rec.replaced_ir_parts_quantity, v_delimiter, i) !=1 THEN
                                                         v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_79');
                                               END IF;
                                              ...
                                              IF common_utils.get_delimited_value(each_rec.replaced_ir_parts_quantity, v_delimiter, i) !=1 THEN
                                                    v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_86');
                                              END IF;
                ...
                ELSE           
                              ...
                              END IF;
                              IF each_rec.replaced_ir_parts_quantity IS NOT NULL AND common_utils.get_delimited_value(each_rec.replaced_ir_parts_quantity, v_delimiter, i) !=1 THEN
                                                         v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_89');
                              END IF;
                              -- Non serialized parts claim hosted on non-serialized machine/competitor model cannot have more than one part off/part on
                              IF UPPER(each_rec.Part_item_Number) IS NOT NULL THEN
                                   IF each_rec.Is_Part_Installed_on_TKTSA in ('N') OR each_rec.Is_Serialized in ('N') THEN                                         
                                         IF Common_Utils.count_delimited_values(each_rec.Replaced_IR_Parts, v_delimiter) >1 THEN
                                            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_96');
                                         END IF;
                                         IF common_utils.count_delimited_values(each_rec.Installed_IR_Parts, v_delimiter) > 1 THEN 
                                            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_97');
                                         END IF;
                                   END IF;
                              END IF;
