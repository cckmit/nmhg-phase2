  ...
  CREATE OR REPLACE PACKAGE BODY "TWMS_TKOWNER"."COMMON_VALIDATION_UTILS" AS 
 ...
 RETURN BOOLEAN
  ...
  BEGIN 
   SELECT m.NAME     
    INTO v_model
    From Item_Group M, Item_Group P,item_group p2
    Where Lower(m.group_code) = Lower(Ltrim(Rtrim(p_model_code))) 
    And Lower(P2.Name) = Lower(Ltrim(Rtrim(p_product)))     
     and lower(m.business_unit_info) = lower(p_business_unit_name)
     and lower(p.business_unit_info) = lower(p_business_unit_name)
     And M.Is_Part_Of = P.Id 
     And P.Is_Part_Of = P2.Id
     And P2.D_Active = 1
     and m.item_group_type = 'MODEL' and m.d_active = 1 AND ROWNUM = 1;
    RETURN TRUE;
  EXCEPTION 
