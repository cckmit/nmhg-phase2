...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF (EACH_REC.ACTION_PERFORMED = 'APPROVED' AND EACH_REC.GOODWILL_POLICY_CODE IS NOT NULL) THEN
        SELECT COMMON_UTILS.ADDERRORMESSAGE(V_ERROR_CODE, 'RE011')
        INTO V_ERROR_CODE
        FROM DUAL
        WHERE NOT EXISTS
          (SELECT 1
  ...
  LOOP
    BEGIN
      V_ERROR_CODE := NULL;
      SELECT COMMON_UTILS.ADDERRORMESSAGE(V_ERROR_CODE, 'RE021')
      INTO V_ERROR_CODE
      FROM DUAL
      WHERE EXISTS
      ...
      IF (INSTR(NVL(V_ERROR_CODE, 'X'), 'RE021') = 0) THEN
        SELECT COMMON_UTILS.ADDERRORMESSAGE(V_ERROR_CODE, 'RE023')
        INTO V_ERROR_CODE
        FROM DUAL
        WHERE NOT EXISTS
          (SELECT 1
        ...
        IF (INSTR(NVL(V_ERROR_CODE, 'X'), 'RE023') = 0) THEN
          SELECT COMMON_UTILS.ADDERRORMESSAGE(V_ERROR_CODE, 'RE030')
          INTO V_ERROR_CODE
          FROM DUAL
          WHERE EXISTS
