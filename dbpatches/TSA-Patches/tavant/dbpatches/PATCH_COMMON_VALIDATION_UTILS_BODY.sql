...
PACKAGE BODY                 "COMMON_VALIDATION_UTILS" AS 
 ...
 --TO CHECK WHETHER THE GIVEN MODEL AND PRODUCT IS VALID IN THE SYSTEM OR NOT
  ...
  v_model VARCHAR2(255) := NULL;
  BEGIN
        -- Check whether the model code belongs to the product
      SELECT A.NAME
      INTO V_MODEL
      FROM
        (SELECT IG.NAME,
        ...
        FROM ITEM_GROUP IG
          ...
          START WITH
          (
            IG.NAME                 = UPPER(LTRIM(RTRIM(p_product)))
      ...
      AND A.NAME               = UPPER(LTRIM(RTRIM(p_model)))
 ...
 --TO CHECK WHETHER THE GIVEN MODEL Code AND PRODUCT IS VALID IN THE SYSTEM OR NOT
  ...
  BEGIN
    -- Check whether the model code belongs to the product
      ...
      WHERE A.ITEM_GROUP_TYPE  = 'MODEL'
      AND A.GROUP_CODE         = UPPER(LTRIM(RTRIM(p_model_code)))
      AND A.BUSINESS_UNIT_INFO = p_business_unit_name
      AND ROWNUM               = 1; 
--    SELECT m.NAME 
--    INTO v_model
...
--    and m.item_group_type = 'MODEL' and m.d_active = 1 AND ROWNUM = 1;
 ...
 --TO CHECK WHETHER THE GIVEN ITEM NUMBER IS VALID IN THE SYSTEM OR NOT
  ...
  --TO CHECK WHETHER THE GIVEN PART SERIAL NUMBER IS VALID IN THE SYSTEM OR NOT
  FUNCTION isValidPartSerialNumber(p_serial_number VARCHAR2, p_business_unit_name VARCHAR2)
  RETURN BOOLEAN
  IS
  v_item_id NUMBER := NULL;  
  BEGIN
    -- Check whether the part serial  number is exist at the System or not
    -- Check for Proper Item number with the given Business Unit with OEM manufactured
    SELECT i.ID 
    INTO v_item_id
 ...
 RETURN BOOLEAN
  ...
  END isValidInventory;
   ...
   --TO CHECK WHETHER THE GIVEN CONTAINER NUMBER IS VALID IN THE SYSTEM OR NOT
...
FUNCTION isValidActionCode (p_action_code VARCHAR2)
RETURN BOOLEAN
IS
    v_id       NUMBER;
BEGIN
    SELECT ID INTO V_ID FROM ACTION_DEFINITION 
    WHERE LOWER(code) = LOWER(p_action_code) AND ROWNUM=1;
    RETURN TRUE;
EXCEPTION
    WHEN OTHERS THEN
        RETURN FALSE;
...
IS
  v_id         NUMBER := NULL;
BEGIN
      select COUNT(1) into  v_id from COSTCAT_APPL_PRODUCTS cp, COST_CATEGORY c where cp.cost_category = c.id and c.code = p_cost_cat_code;
      IF v_id=0 then
          RETURN TRUE;
      ELSE    
        SELECT COUNT(1) into v_id from COSTCAT_APPL_PRODUCTS cp, COST_CATEGORY c where cp.cost_category = c.id and c.code = p_cost_cat_code and cp.item_group = p_product;
         IF v_id >0 then
            RETURN TRUE;
         else
            RETURN FALSE;
