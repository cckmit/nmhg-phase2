--Purpose    : Patch for config param to Allow warranty registration across other dealers stock inventory 
...
VALUES 
(CONFIG_PARAM_SEQ.NEXTVAL, 'Specify if dealers should be allowed to register units in other dealers stock', 'Allow warranty registration across other dealers stock inventory', 'allowWntyRegOnOthersStock', 'boolean', current_date, 'TSA-Configuration', current_date, NULL, current_timestamp, current_timestamp, 
...
VALUES
(CONFIG_VALUE_SEQ.NEXTVAL,1,'',(SELECT ID FROM CONFIG_PARAM WHERE NAME = 'allowWntyRegOnOthersStock'), CURRENT_DATE, '',CURRENT_DATE, '',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1, 'Thermo King TSA', 
(SELECT ID FROM CONFIG_PARAM_OPTION WHERE VALUE = 'true'))
/
COMMIT
/