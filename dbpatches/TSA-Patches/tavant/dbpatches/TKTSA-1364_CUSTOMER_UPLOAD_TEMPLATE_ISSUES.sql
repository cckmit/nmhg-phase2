...
BEGIN
    ...
    BEGIN
        ...
        IF v_is_update THEN
			...
			select ORG_ADDRESSES into v_org_addr_id from ORGANIZATION_ORG_ADDRESSES where organization =v_party_id;
            
			If Upper(Each_Rec.Dealer_Site)Is Not Null Then
              Update Organization_Address Set 
              location = each_rec.addressline1 ||COALESCE('-'||each_rec.addressline2,'') || COALESCE('-'||each_rec.city,'') 
        ...
        IF NOT v_is_update THEN
            ...
            VALUES
                ...
                EACH_REC.EMAIL, EACH_REC.PHONE, EACH_REC.STATE, EACH_REC.ZIP_CODE, UPPER(EACH_REC.STATUS), v_d_active, 0,
            ...
            Values (V_Party_Id, V_Address_Id);
             ...
             INSERT INTO organization_address (id, location, site_number)VALUES( V_Address_Id,
              each_rec.addressline1 ||COALESCE('-'||each_rec.addressline2,'') || COALESCE('-'||each_rec.city,'') 
                || COALESCE('-'||each_rec.state,'') ||COALESCE('-'||each_rec.zip_code,'') || COALESCE('-'||each_rec.country,''),
