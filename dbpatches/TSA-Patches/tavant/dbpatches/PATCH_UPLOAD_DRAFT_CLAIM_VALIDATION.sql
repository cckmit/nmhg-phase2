    ...
    IF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE SERIALIZED', 'PARTS WITH HOST', 'FIELDMODIFICATION') THEN
        IF EACH_REC.SERIAL_NUMBER IS NULL THEN
			  ...
			  IF EACH_REC.CONTAINER_NUMBER IS NULL THEN
					v_error_code := Common_Utils.addErrorMessage(v_error_code, 'CON_DC065');        
			  ELSIF NOT (COMMON_VALIDATION_UTILS.isValidInventoryWithConNum(EACH_REC.CONTAINER_NUMBER, v_bu_name)) THEN
					v_error_code := Common_Utils.addErrorMessage(v_error_code, 'CON_DC005');
				ELSIF UPPER(each_rec.claim_type) = 'FIELDMODIFICATION' AND v_valid_campaign_code = TRUE AND NOT
					...
					v_error_code := Common_Utils.addErrorMessage(v_error_code, 'CON_DC064');
				ELSE
					...
					ELSIF v_count > 1 THEN
						IF EACH_REC.MODEL_NUMBER IS NULL THEN
							v_error_code := Common_Utils.addErrorMessage(v_error_code, 'CON_DC006');
						ELSIF NOT (COMMON_VALIDATION_UTILS.isValidModel(EACH_REC.MODEL_NUMBER, v_bu_name)) THEN
							v_error_code := Common_Utils.addErrorMessage(v_error_code, 'CON_DC007');
						ELSE
