...
AS
    ...
    -- Added for DCAP model category field validation
    ...
    BEGIN
        ...
        Else
          Begin
              ...
              -- checking item_number with BU and itemType
              SELECT id,
                product,
                model
              INTO v_type_of,
              ...
              And   Owned_By 		                  = V_Oem_Id 
              AND   ROWNUM                        = 1;
            Exception
              When No_Data_Found Then
                v_error_code := common_utils.addErrorMessage(v_error_code, 'IB055');
          end;
            SELECT i.model 
            INTO v_model_id
            FROM ITEM i, PARTY p 
        ...
        END IF;
        --Add error message for validating dcap_flag field
      ...
      -- Below validations which are against to DCAP dealers and dealer group
       If Each_Rec.Dcap_Model_Code Is Not Null Then          
            -- checking model category
            Begin            
              Select id
              Into  v_model_cat_id
              From  Model_Category
              ...
              -- checking dealer group based on scheme, dealer, BU
              Begin                
                Select Ds.Id
                ...
                -- Checking for price_matrix entry based on model_category, dealer_group, currency, and inventory shipment dates
                Begin
                  Select Id,
                         Grid_Amount,
                         Grid_Amount_Curr
