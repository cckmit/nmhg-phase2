...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      ELSIF NOT common_validation_utils.isValidModelCodeForProduct( each_rec.field_model, each_rec.product_code, v_bu_name) THEN
        ...
        BEGIN
          ...
          FROM
            ...
            FROM ITEM_GROUP IG
              ...
              START WITH
              (
                IG.NAME                 = UPPER(LTRIM(RTRIM(each_rec.product_code)))
              AND IG.BUSINESS_UNIT_INFO = v_bu_name
          ...
          AND A.NAME               = UPPER(LTRIM(RTRIM(each_rec.field_model)))
          AND A.BUSINESS_UNIT_INFO = v_bu_name
          AND ROWNUM               = 1;
          ...
          WHERE UPPER(name)    =UPPER(each_rec.action);
