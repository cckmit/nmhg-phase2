            ...
            IF V_SERIAL_ID > 0 THEN
              ...
              IF UPPER
                ...
                BEGIN
                  ...
                  INTO V_MARKET_ID
                  FROM MARKET_TYPE
                  WHERE UPPER(TITLE)     = UPPER(EACH_REC.MARKET_TYPE)
                  AND D_ACTIVE           = 1
                  AND BUSINESS_UNIT_INFO = EACH_REC.BUSINESS_UNIT_INFO;
                  SELECT ID
                  INTO V_COMPETITION_TYPE_ID
                  FROM COMPETITION_TYPE
                  WHERE UPPER(TYPE)      = UPPER(EACH_REC.COMPETITION_TYPE)
                  AND D_ACTIVE           = 1
                  AND BUSINESS_UNIT_INFO = EACH_REC.BUSINESS_UNIT_INFO;
                  SELECT ID
                  INTO V_TRANSACTION_TYPE_ID
                  FROM TRANSACTION_TYPE
                  WHERE UPPER(TYPE)      = UPPER(EACH_REC.TRANSACTION_TYPE)
                  AND D_ACTIVE           = 1
                  AND BUSINESS_UNIT_INFO = EACH_REC.BUSINESS_UNIT_INFO;
                  SELECT ID
                  INTO V_COMPETITOR_MODEL_ID
                  FROM COMPETITOR_MODEL
                  WHERE UPPER(MODEL)     = UPPER(EACH_REC.MODEL_NUMBER)
                  AND D_ACTIVE           = 1
                  AND BUSINESS_UNIT_INFO = EACH_REC.BUSINESS_UNIT_INFO;
                  SELECT ID
                  INTO V_COMPETITOR_MAKE_ID
                  FROM COMPETITOR_MAKE
                  WHERE UPPER(MAKE)      = UPPER(EACH_REC.COMPETITOR_MAKE)
                  AND D_ACTIVE           = 1
                  AND BUSINESS_UNIT_INFO = EACH_REC.BUSINESS_UNIT_INFO;
                  INSERT
              ...
              LOOP
                UPLOAD_WARRANTY_COVERAGE(EACH_PLAN.ID, V_WARRANTY_ID, V_SERIAL_ID, V_SHIP_DATE, TO_DATE(EACH_REC.DELIVERY_DATE,'YYYYMMDD'), EACH_REC.BUSINESS_UNIT_INFO, V_LAST_UPDATED_BY, V_IS_RED_CVG);
              ...
              IF EACH_REC.ADDITIONAL_APPLICABLE_POLICIES IS NOT NULL THEN
                BEGIN
                  ...
                  LOOP
                    ...
                    UPLOAD_WARRANTY_COVERAGE(V_POLICY_DEFN_ID, V_WARRANTY_ID, V_SERIAL_ID, V_SHIP_DATE, TO_DATE(EACH_REC.DELIVERY_DATE,'YYYYMMDD'), EACH_REC.BUSINESS_UNIT_INFO, V_LAST_UPDATED_BY, V_IS_RED_CVG);
                    IF V_IS_RED_CVG  = 'TRUE' THEN
                      V_IS_REQ_EXTN := 'TRUE';
                    END IF;
                  END LOOP;
              ...
              LOOP
                UPLOAD_WARRANTY_COVERAGE(EACH_EXTND_POLICY.POLICY, V_WARRANTY_ID, V_SERIAL_ID, V_SHIP_DATE, TO_DATE(EACH_REC.DELIVERY_DATE,'YYYYMMDD'), EACH_REC.BUSINESS_UNIT_INFO, V_LAST_UPDATED_BY, V_IS_RED_CVG);
                IF V_IS_RED_CVG  = 'TRUE' THEN
                  V_IS_REQ_EXTN := 'TRUE';
                END IF;
              END LOOP;
