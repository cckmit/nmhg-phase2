...
CURSOR ALL_REC IS
    ...
    v_ac_input      		    NUMBER         := 0;
    v_id                    NUMBER := NULL; 
    sericing_location        VARCHAR2(255) := NULL; 
BEGIN
  ...
  LOOP
	...
	-- VALIDATE THAT CLAIM TYPE IS NOT NULL AND AN ACCEPTED VALUE
		 IF EACH_REC.SERVICING_LOCATION_ID IS NOT NULL
		 THEN
      BEGIN
        select location into sericing_location from organization_address where site_number=EACH_REC.SERVICING_LOCATION_ID;
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
              v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC_99');
     	END;
		 END IF;
    ...
    IF UPPER(EACH_REC.CLAIM_TYPE) IN ('PARTS WITH HOST', 'PARTS WITHOUT HOST')  THEN
        IF EACH_REC.PART_ITEM_NUMBER IS NULL THEN
           -- v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC014');
            ...
            ELSE
                 SELECT model.id,i.item_number into v_model_part, v_item_number
...
--        dbms_output.put_line(v_job_code);
--        dbms_output.put_line(v_bu_name);
