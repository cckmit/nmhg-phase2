...
--            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC012');
        IF v_flag THEN
             ...
             WHERE m.business_unit_info = v_bu_name            
                ...
                select is_part_of INTO v_product from item_group pro,party pa where pro.id in 
                ( select is_part_of from item_group m where name like each_rec.model_number)AND 
                pro.business_unit_info = v_bu_name
                AND pro.item_group_type = 'PRODUCT FAMILY'
                AND pa.name = 'OEM'  AND pro.d_active = 1 AND pa.d_active = 1; 
          END IF;       
    ...
    IF UPPER(EACH_REC.CLAIM_TYPE) IN ('PARTS WITH HOST', 'PARTS WITHOUT HOST')  THEN
        IF EACH_REC.PART_ITEM_NUMBER IS NULL THEN
           -- v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC014');
            ...
            SELECT product.id into v_product
						FROM ITEM_GROUP product, INVENTORY_ITEM ii, ITEM i
						WHERE ii.of_type = i.id AND I.product = product.ID AND 
						lower(ii.serial_number) = lower(trim(EACH_REC.PART_SERIAL_NUMBER)) AND 
						lower(ii.business_unit_info) = lower(trim(v_bu_name)) AND ROWNUM = 1;
        ...
        ELSE
            ...
            SELECT product.id into v_product
				    FROM ITEM_GROUP product, ITEM i
				    WHERE i.product = product.ID AND 
				    lower(i.item_number) = lower(trim(EACH_REC.PART_ITEM_NUMBER)) AND 
				    lower(i.business_unit_info) = lower(trim(v_bu_name)) AND ROWNUM = 1; 
        END IF;
    END IF;
     EXCEPTION
        WHEN NO_DATA_FOUND THEN
