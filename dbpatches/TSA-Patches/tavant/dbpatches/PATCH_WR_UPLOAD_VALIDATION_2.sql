    ...
    LOOP
      BEGIN
        ...
        IF V_ADD_INFO_APPLICABLE        = 'TRUE' THEN
          ...
          ELSIF upper(EACH_REC.FIRST_TIME_OWNER)   = 'NO' AND EACH_REC.IF_PREVIOUS_OWNER IS NULL THEN
            V_ERROR_CODE                          := common_utils.addErrorMessage(V_ERROR_CODE, 'WR028');
          ELSIF (UPPER(EACH_REC.IF_PREVIOUS_OWNER) = UPPER(('Switching to ' || EACH_REC.BUSINESS_UNIT_INFO)) OR UPPER(EACH_REC.IF_PREVIOUS_OWNER) = UPPER(('Continuing with ' || EACH_REC.BUSINESS_UNIT_INFO)) OR UPPER(EACH_REC.IF_PREVIOUS_OWNER) = UPPER('Unknown/Not Provided')) THEN
        ...
        IF EACH_REC.COMPONENT_PART_NUMBER IS NOT NULL THEN
          ...
          IF EACH_REC.COMPONENT_SERIAL_NUMBER IS NOT NULL THEN
            ...
            IF EACH_REC.COMPONENT_INSTALLATION_DATE IS NOT NULL THEN
              ...
              LOOP
                IF NOT COMMON_UTILS.ISVALIDDATE(V_COMP_INSTALL_ARRAY(I)) OR TO_DATE(V_COMP_INSTALL_ARRAY(I), 'YYYYMMDD') < V_SHIP_DATE OR TO_DATE(V_COMP_INSTALL_ARRAY(I), 'YYYYMMDD') < V_BUILD_DATE THEN
        ...
        AND SP.ID                        = BOM.ORG
        AND BOM.BU                       = EACH_REC.BUSINESS_UNIT_INFO;
        --VALIDATE IF ANY ALL ELIGIBLE POLICY PLANS ARE APPLIED ELSE SET ERROR CODE
        OPEN ALL_ELIGIBLE_POLICY_PLANS(V_PRODUCT, V_MODEL, EACH_REC.BUSINESS_UNIT_INFO, SYSDATE, TO_NUMBER(V_HOURS_ON_SERV), V_CONDITION, V_CERT_STATUS, V_INSTALL_DEALER_ID);
