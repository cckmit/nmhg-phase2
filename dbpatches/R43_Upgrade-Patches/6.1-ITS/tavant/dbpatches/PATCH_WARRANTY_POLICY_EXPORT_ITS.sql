...
TRIGGER WARRANTY_POLICY_TRIGGER AFTER
  INSERT OR
  UPDATE  ON POLICY_DEFINITION FOR EACH ROW
DECLARE 
  ...
  warranty_type varchar2(50);
  months_frm_ship number :=0;
  service_hrs_covered number :=0;
  BEGIN
    IF :NEW.business_unit_info = 'ITS' THEN
      ...
      -- Checking months_frm_shipment has any value or not
      if (:new.months_frm_shipment is not null ) then
        months_frm_ship := :new.months_frm_shipment;
      end if;
      if (:new.service_hrs_covered is not null ) then
        service_hrs_covered := :new.service_hrs_covered;
      end if;
