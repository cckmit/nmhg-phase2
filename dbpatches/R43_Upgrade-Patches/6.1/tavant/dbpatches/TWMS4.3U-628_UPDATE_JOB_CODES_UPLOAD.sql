    ...
    ELSE
        ...
        ELSIF v_valid_action AND v_valid_bu THEN
            ...
            WHILE v_index > 0 LOOP
                ...
                END IF;
            END LOOP;
			v_job_code := v_job_code || '-' || each_rec.action;
		...
		ELSIF v_level = 'PRODUCT' THEN
			...
			WHERE an.id=sp.defined_for AND sp.definition=spd.id 
				...
				(SELECT a.id FROM assembly a WHERE a.fault_code IS NOT NULL
					...
					WHERE fsa.failure_structure=fs.id AND fs.for_item_group IN (
						SELECT t.id FROM (
							SELECT ig.item_group_type,ig.id
							FROM item_group ig
								CONNECT BY PRIOR ig.id = ig.is_part_of
								START WITH (ig.is_part_of = v_item_group_id AND ig.d_active = 1)
						)t 
						WHERE t.item_group_type='MODEL'))
				);
		...
		(SELECT a.id FROM assembly a WHERE a.fault_code IS NOT NULL
			...
			WHERE fsa.failure_structure=fs.id AND fs.for_item_group IN (
				SELECT t.id FROM (
					...
					FROM item_group ig
						CONNECT BY PRIOR ig.id = ig.is_part_of
						START WITH (ig.is_part_of = p_item_group AND ig.d_active = 1)
				)t 
				WHERE t.item_group_type='MODEL'
			))
		);
