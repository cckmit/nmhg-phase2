...
FOR each_rec IN all_rec LOOP
    ...
    ELSE
        BEGIN
            ...
            WHERE c.business_unit_info=v_bu_name  AND UPPER(r.recovery_claim_number)=UPPER(each_rec.claim_number)
                ...
                AND r.claim=c.id AND r.id=(SELECT MAX(id) FROM recovery_claim WHERE claim=c.id);--TKTSA 870 Updated 
        EXCEPTION
    ...
    BEGIN
       ...
       WHERE   hpr.id =opr.oem_replaced_parts
            ...
            AND hpr.service_detail=v_service_id;--TKTSA 870 Updated 
    ...
    IF each_rec.cost_price IS NULL or each_rec.cost_price < 0  THEN
