--Purpose    : Scripts for creating VENDOR_RECOVERY_EXTRACT view, changes made as a part of request sent by Single Instance DEV Team
--Created On : 16-Mar-2011
--Created By : Kuldeep Patil
--Impact     : None

CREATE OR REPLACE FORCE VIEW VENDOR_RECOVERY_EXTRACT (BUSINESS_UNIT_INFO, ID, CLAIM_NUMBER, CLAIM_TYPE, RECOVERY_CLAIM_STATE, REC_CLAIM_CREATED_DATE, REC_CLAIM_UPDATED_DATE, REC_CLAIM_UPDATED_BY, REC_CLAIM_MODIFIED_DATE, FAILURE_DATE, REPAIR_DATE, DEALER_NUMBER, DEALER_NAME, CAUSAL_PART_NUMBER, REPLACED_PART_NUMBER, SUPPLIER_NUMBER, SUPPLIER_NAME, SERIAL_NUMBER, MODEL_DESC, BUILD_DATE, INVOICE_DATE, DELIVERY_DATE, JOB_CODE, HOURS_IN_SERVICE, FAULT_FOUND, CAUSED_BY, DEALER_COMMENTS, PROCESSOR_COMMENTS, MATERIAL_COST_TOTAL, NON_TK_PARTS_TOTAL, MATERIAL_PARTS_TOTAL, TOTAL_LABOR_HOURS, LABOR_COST_TOTAL, MISC_COST_TOTAL, TOTAL_ACTUAL_AMT, TOTAL_CONTRACT_AMT, TOTAL_WARRANTY_AMT, SUPPLIER_CURRENCY, CREDIT_MEMO_DATE, CREDIT_MEMO_NUMBER, RECOVERY_COMMENTS, CONTRACT_NAME, REC_CLAIM_ACCEPTANCE_REASON, REC_CLAIM_REJECTION_REASON)
AS
...
c.failure_date,
c.repair_date,
(SELECT service_provider_number FROM service_provider WHERE id = c.for_dealer
) dealer_number,
...
NVL(c.condition_found,'')
|| ' ## '
|| NVL(c.work_performed,'')
|| ' ## '
|| NVL(c.other_comments,'') dealer_comments,
...
(SELECT SUM(li.supplier_cost_amt)
FROM cost_line_item li,
  rec_clm_cost_line_items rli,
  section s
WHERE li.id           =rli.cost_line_items
