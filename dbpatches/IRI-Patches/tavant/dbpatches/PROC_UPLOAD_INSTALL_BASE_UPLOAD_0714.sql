--Purpose    : Handled new columns address_line2 & address_line3
--Author     : raghuram.d
          ...
          IF v_inventory_type='RETAIL' THEN
            ...
            IF UPPER(each_rec.country) = 'US' THEN
                ...
                WHERE c.id = ca.customer AND ca.addresses = a.id
                    ...
                    AND UPPER(a.address_line1) = UPPER(each_rec.address)
            ...
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    ...
                    INSERT INTO address(id,address_line1,country,state,
                        city,zip_code,contact_person_name,email,phone,secondary_phone,
                        version,d_active,d_created_on,d_internal_comments)
                    VALUES(v_cust_addr_id, each_rec.address, UPPER(each_rec.country), UPPER(each_rec.state), 
        ...
        IF each_rec.engine_serial_number IS NOT NULL THEN
            ...
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    ...
                    VALUES(v_datasource_attr, 'EngineSerialNo', 0, 1, v_bu_name, 'InstallBase Upload', sysdate); 
            END;
            ...
            IF UPPER(NVL(each_rec.apply_coverage, 'Y')) = 'Y' THEN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        SELECT ADD_MONTHS(TO_DATE(each_rec.shipment_date, 'YYYYMMDD'), each_plan.months_frm_shipment)-1
                        INTO v_ship_coverage_till_date
                        FROM DUAL;

                        --CALCULATE DATE  INTO  v_coverage_till_date, BY ADDING 'MONTHS FROM DELIVERY'
                        SELECT ADD_MONTHS(TO_DATE(v_warranty_start_date, 'YYYYMMDD'), each_plan.months_frm_delivery)-1
