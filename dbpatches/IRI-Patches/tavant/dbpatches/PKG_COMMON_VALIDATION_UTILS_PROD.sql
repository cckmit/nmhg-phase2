--Purpose    : Package for common validation activities
...
PACKAGE BODY COMMON_VALIDATION_UTILS AS
 ...
 --TO CHECK WHETHER THE GIVEN SUPPLIER IS VALID IN THE SYSTEM OR NOT
  ...
  BEGIN
    ...
    INTO v_supplier_id
    FROM SUPPLIER 
    WHERE lower(supplier_number) = lower(ltrim(rtrim(p_supplier_name))) AND 
    lower(business_unit_info) = lower(ltrim(rtrim(p_business_unit_name))) AND ROWNUM = 1;
    RETURN TRUE;
  EXCEPTION 
  WHEN OTHERS THEN
    RETURN FALSE;
