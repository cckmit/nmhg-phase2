...
BEGIN
    ...
    FROM ITEM_GROUP 
    WHERE UPPER(GROUP_CODE) = UPPER(EACH_REC.ITEM_GROUP_CODE) AND BUSINESS_UNIT_INFO = v_bu
        AND UPPER(ITEM_GROUP_TYPE) = 'MODEL';

    IF EACH_REC.OWNER IS NOT NULL THEN
        BEGIN
        SELECT P.NAME, P.ID INTO v_make_name, v_owner_id
        FROM SUPPLIER S, PARTY P
        WHERE upper(S.SUPPLIER_NUMBER) = upper(EACH_REC.OWNER) AND
            S.ID = P.ID AND ROWNUM = 1;
        EXCEPTION
    ...
    ELSE
        ...
        FROM PARTY P, ORGANIZATION O
        WHERE UPPER(P.NAME) = 'OEM' AND
            P.ID = O.ID;
    END IF;
    ...
    SELECT DECODE(EACH_REC.ITEM_GROUP_CODE,'PART','PART','MACHINE') INTO v_item_type FROM DUAL;
