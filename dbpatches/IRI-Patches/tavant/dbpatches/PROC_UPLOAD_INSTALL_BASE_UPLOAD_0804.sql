--Purpose    : Added end customer number to the IB upload template
--Author     : raghuram.d
...
BEGIN
    ...
    BEGIN
        ...
        IF each_rec.manufacturing_site IS NOT NULL THEN 
            BEGIN
                SELECT id INTO v_manufacturing_site 
                FROM list_of_values 
                WHERE business_unit_info=v_bu_name 
        ...
        IF v_inventory_type='RETAIL' THEN
            ...
            IF each_rec.end_customer_number IS NOT NULL THEN
                ...
                FROM customer c, customer_addresses ca, 
                    address_book_address_mapping abam, address_book ab
            ...
            IF each_rec.end_customer_number IS NOT NULL THEN
                UPDATE party SET name = each_rec.end_customer_name WHERE id = v_end_customer_id;
                UPDATE customer SET 
                ...
                UPDATE address SET
                    address_line1 = each_rec.address_line1,
                ...
                WHEN NO_DATA_FOUND THEN
                    ...
                    EXCEPTION
                        WHEN NO_DATA_FOUND THEN
                            ...
                            INSERT INTO address_book (id, type, version, belongs_to, d_internal_comments, d_active)
                            VALUES (v_address_book_id, 'ENDCUSTOMER', 0, v_dealer_id, 'IB Upload', 1);
                    END;
                    ...
                    INSERT INTO customer (id, company_name, individual, locale, customer_id)
                    VALUES(v_end_customer_id, each_rec.end_customer_name, 0, each_rec.prefered_language, each_rec.end_customer_number);
