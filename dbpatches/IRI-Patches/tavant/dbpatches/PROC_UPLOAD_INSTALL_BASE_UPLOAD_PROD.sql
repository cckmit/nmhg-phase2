-- Purpose    : Used to upload install base data
-- Author     : Jhulfikar Ali. A
-- Created On : 14-Jan-08

CREATE OR REPLACE
PROCEDURE UPLOAD_INSTALL_BASE_UPLOAD AS
    ...
    LOOP
	   BEGIN
        ...
        WHEN OTHERS THEN
          v_warranty_id := 0;
				...
				BEGIN
					...
					WHERE TRNX_TYPE_KEY = DECODE(EACH_REC.STOCK_OR_RETAIL, 'STOCK', 'IB','RETAIL', 'DR');
				...
				BEGIN
					...
					IF UPPER(EACH_REC.STOCK_OR_RETAIL)='STOCK'
					THEN
						SELECT ID
						INTO v_seller_id
						FROM PARTY
          ...
          IF UPPER(EACH_REC.STOCK_OR_RETAIL)='RETAIL'
          THEN
        ...
        (
            ...
            v_seller_id,
            v_trans_type,
            DECODE(UPPER(EACH_REC.STOCK_OR_RETAIL),'RETAIL',v_seller_id,'STOCK',v_buyer_id), --CHANGE IT ACCORDINGLY LATER,
            DECODE(UPPER(EACH_REC.STOCK_OR_RETAIL),'RETAIL',2,'STOCK',1),
            'ACTIVE'
         );
         ...
         THEN
          ...
          (
            ...
            'Cash Sales',  --need to ask, why specifically given this value only, whereas other options r also there
            '',
            NULL,
            'UNKNOWN/NOT PROVIDED',						
            'UNKNOWN/NOT PROVIDED'
          ...
          (
            SELECT 
              ...
              'BILLING',
              0, 
              ZIP_CODE,NULL,NULL,NULL,NULL,NULL,NULL,NULL
            FROM ADDRESS
        ...
        --GET ALL THE APPLICABLE POLICY CODES ON THIS PARTICULAR INVENTORY SERIAL NUMBER
          ...
          BEGIN
            ...
            BEGIN
              ...
              --CONDITION 2
              IF v_current_date >= EACH_REC.WARRANTY_START_DATE
