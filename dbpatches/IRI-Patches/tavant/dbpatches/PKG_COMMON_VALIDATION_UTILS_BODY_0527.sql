...
create or replace PACKAGE BODY COMMON_VALIDATION_UTILS AS
 ...
 --TO CHECK WHETHER THE GIVEN ITEM NUMBER IS VALID IN THE SYSTEM OR NOT
  ...
  BEGIN
    ...
    FROM business_unit
    WHERE 
    name = p_business_unit_name AND ROWNUM = 1;
    RETURN TRUE;
  EXCEPTION 
  WHEN OTHERS THEN
    RETURN FALSE;
...
BEGIN
    ...
    FROM i18nlov_text t, list_of_values l
    WHERE t.description = p_lov AND t.list_of_i18n_values = l.id
        AND UPPER(l.business_unit_info) = UPPER(p_business_unit_info)
        AND UPPER(t.locale) = UPPER(p_locale)
        AND l.type = 'SMRREASON'
        AND l.d_active = 1 AND ROWNUM = 1 ;
    RETURN TRUE;
EXCEPTION
    WHEN OTHERS THEN
        RETURN FALSE;
