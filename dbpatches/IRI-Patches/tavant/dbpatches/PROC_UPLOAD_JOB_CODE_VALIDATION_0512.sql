  ...
  LOOP
    BEGIN
        ...
        END;
                ...
                -- ERROR CODE: JC0014
				-- VALIDATE THAT MODEL IS VALID FOR THE PRODUCT
				-- REASON FOR ERROR: MODEL FOR THE PRODYCR SHOULD BE VALID
				BEGIN
					 --CHECK IN BASE TABLE IF THIS MODEL NUMBER ALREADY EXIST           
           IF NOT (COMMON_VALIDATION_UTILS.isValidModelForProduct(EACH_REC.FIELD_MODEL, EACH_REC.PRODUCT_CODE, EACH_REC.BUSINESS_UNIT_NAME)) THEN
            v_error_code := Common_Utils.addError(v_error_code, 'JC0014');
           END IF;
        END;
				...
				BEGIN
					 ...
					 WHERE
						   m.ITEM_GROUP_TYPE = 'MODEL' AND
					 	   UPPER(m.name) = UPPER(EACH_REC.FIELD_MODEL) AND m.BUSINESS_UNIT_INFO = EACH_REC.BUSINESS_UNIT_NAME
                           AND m.is_part_of=p.id
                           AND UPPER(p.name) = UPPER(EACH_REC.PRODUCT_CODE) AND p.BUSINESS_UNIT_INFO = EACH_REC.BUSINESS_UNIT_NAME;
