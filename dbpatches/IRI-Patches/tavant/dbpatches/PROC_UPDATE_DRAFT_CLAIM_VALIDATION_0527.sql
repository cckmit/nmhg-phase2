...
BEGIN
    ...
    BEGIN
        SELECT u.locale, u.login INTO v_user_locale, v_dealer 
  ...
  LOOP
	...
	BEGIN
     ...
     NOT (COMMON_VALIDATION_UTILS.isValidBusinessUnitName(EACH_REC.BUSINESS_UNIT_NAME))
		...
		ELSIF NOT (COMMON_VALIDATION_UTILS.isUserBelongsToBU(EACH_REC.BUSINESS_UNIT_NAME,v_dealer)) THEN
    ...
    IF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE SERIALIZED', 'PARTS WITH HOST', 'FIELDMODIFICATION') THEN
        ...
        ELSIF NOT (COMMON_VALIDATION_UTILS.isValidInventory(EACH_REC.SERIAL_NUMBER, EACH_REC.BUSINESS_UNIT_NAME)) THEN
            v_error_code := Common_Utils.addErrorMessage(v_error_code, 'DC005');
        ELSE
            SELECT COUNT(1) INTO v_count FROM inventory_item 
            WHERE business_unit_info = each_rec.business_unit_name
            ...
            ELSIF v_count > 1 THEN
                ...
                ELSIF NOT (COMMON_VALIDATION_UTILS.isValidModel(EACH_REC.MODEL_NUMBER, EACH_REC.BUSINESS_UNIT_NAME)) THEN
                ...
                ELSE
                    ...
                    AND inv.business_unit_info = each_rec.business_unit_name
    ...
    IF UPPER(EACH_REC.CLAIM_TYPE) IN ('MACHINE NON SERIALIZED') THEN
        ...
        ELSIF NOT (common_validation_utils.isValidItemNumber(each_rec.item_number, each_rec.business_unit_name)) THEN
        ...
        ELSIF v_flag THEN
            ...
            WHERE (lower(i.alternate_item_number) = lower(ltrim(rtrim(each_rec.item_number))) OR 
                ...
                AND i.business_unit_info = each_rec.business_unit_name
    ...
    IF UPPER(EACH_REC.CLAIM_TYPE) IN ('PARTS WITH HOST', 'PARTS WITHOUT HOST')  THEN
        ...
        ELSIF NOT (COMMON_VALIDATION_UTILS.isValidItemNumber(EACH_REC.PART_ITEM_NUMBER, EACH_REC.BUSINESS_UNIT_NAME)) THEN
  ...
  END IF;
	...
	BEGIN
		 IF EACH_REC.SMR_CLAIM IS NOT NULL AND each_rec.smr_claim = 'Y' THEN
            ...
            ELSIF NOT COMMON_VALIDATION_UTILS.isValidSMRReason(each_rec.reason_for_smr_claim, v_user_locale, each_rec.business_unit_name) THEN
	...
	BEGIN
     ...
     NOT COMMON_VALIDATION_UTILS.isValidFaultCodeForModelId(v_model, EACH_REC.fault_location, EACH_REC.BUSINESS_UNIT_NAME)
		 THEN
	...
	BEGIN
     ...
     NOT COMMON_VALIDATION_UTILS.isValidJobCodeForModelId(v_model, EACH_REC.JOB_CODE, EACH_REC.BUSINESS_UNIT_NAME)
		 THEN
	...
	BEGIN
		 IF v_model IS NOT NULL  AND EACH_REC.FAULT_FOUND IS NOT NULL THEN
            IF NOT COMMON_VALIDATION_UTILS.isValidFaultFoundForModelId(v_model, EACH_REC.FAULT_FOUND, EACH_REC.BUSINESS_UNIT_NAME)
		    THEN
	...
	BEGIN
     ...
     NOT COMMON_VALIDATION_UTILS.isValidRootCauseForModelId(v_model, EACH_REC.FAULT_FOUND, EACH_REC.failure_detail, EACH_REC.BUSINESS_UNIT_NAME)
		 THEN
    ...
    BEGIN
		 IF each_rec.technician_id IS NOT NULL AND 
            NOT COMMON_VALIDATION_UTILS.isValidTechnician(each_rec.technician_id, v_dealer, each_rec.business_unit_name)
		 THEN
