...
<tr>
	...
	<table width="98%" cellpadding="0" cellspacing="0" border="0">
		<tr>
			...
			<div dojoType="twms.widget.TitlePane"
				...
				<tbody>
					<tr>
						<td width="20%" class="labelStyle" nowrap="nowrap" style="border:0"><s:text
							name="label.common.serialNumber" />:</td>

						<td class="" width="37%" style="border:0"><s:hidden
							  ...
							  <script type="text/javascript">							  
               					 dojo.addOnLoad(function() {
               						var index = '<s:property value="%{#nListIndex}" />';               											    				
                       			 	var url= "listAllStockSerialsStartingWith.action?selectedBusinessUnit="+dojo.byId("selectedBusinessUnit").value+
						...
						<td class="" width="35%" style="border:0"><s:label
							id="%{qualifyId(\"model\")}"
					...
					<tr>
						<td width="20%" class="labelStyle" style="border:0" nowrap="nowrap"><s:text
							name="label.common.product" />:</td>
						<td class="" width="37%" style="border:0"><s:label
					...
					<tr>
						...
						<td class="" width="37%" style="border:0"><s:label
						 ...
						 <s:if test="!buConfigAMER || (!inventoryItem.latestWarranty.status.status.equals('Deleted')
						 								&& inventoryItem.latestWarranty.pdiGenerated)">
						<td width="20%" class="labelStyle" nowrap="nowrap" style="border:0"><s:text
							name="label.deliveryDate" /></td>
						<td class="" width="35%" style="border:0">
						<sd:datetimepicker name='%{#nListName}.warrantyDeliveryDate' value='%{warrantyDeliveryDate}' id='%{qualifyId("deliveryDate")}' />
					...
					<tr>
						<s:if test="isInstallingDealerEnabled()">
							...
							<td class="" width="37%" style="border:0">
							<sd:datetimepicker name='%{#nListName}.installationDate' value='%{installationDate}' id='%{qualifyId("installationDateForUnit")}' />
	            	...
	            	getAllPolicies('<s:property value="%{#nListIndex}"/>','<s:property value="%{#nListName}" />');	            		
						...
						<td class="" width="35%" style="border:0"><s:textfield
		...
		<tr>
			<td style="border:0">
				...
				<div dojoType="dijit.layout.ContentPane" 
					id='<s:property value="qualifyId(\"policyDetails\")" />'>
					<s:if
					test="!availablePolicies.empty">					
					<jsp:include page="policy_list.jsp" flush="true"/>			
				</s:if></div> </div>
		...
		<tr>
			<td style="border:0">
			<div dojoType="twms.widget.TitlePane" id='<s:property value="qualifyId(\"marketingInformation\")" />'
				...
				<div dojoType="dojox.layout.ContentPane" executeScripts="true"
					...
					<table width="100%"
				id="<s:property value="qualifyId(\"marketInformationTable\")" />" border="0">
		...
		<tr>
			...
			<div dojoType="twms.widget.TitlePane" title="<s:text name="label.warranty.supportDocs"/>" labelNodeClass="section_header" open="true">
				<s:if test = "buConfigAMER && loggedInUserADealer">
					<div style="color:red" id="demoTruckAuthorizationMessage">
						<s:text name="message.demoTruck.authorization" />
					</div>
				</s:if>
	...
	<td width="5%">
		<div class="nList_delete" id="nListDelete_id_<s:property value="%{#nListIndex}" />"> </div>
	</td>
...
var params={serialNumber:serialNumber,"inventoryItemMappingIndex":index,"type":"STOCK"};
...
dojo.addOnLoad(function(){	
	...
	toggleMessageForDemoTruckMoreThan80Hours(<s:property value="%{#nListIndex}" />);
