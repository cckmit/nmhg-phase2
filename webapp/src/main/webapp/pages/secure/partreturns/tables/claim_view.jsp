...
<table border="0" cellspacing="0" cellpadding="0" class="grid borderForTable">	
  ...
  <tr>
	...
	<th width="10%" valign="middle" class="colHeader"><s:text name="columnTitle.partReturnConfiguration.partNumber" /></th>
    ...
    <th width="6%%" valign="middle" class="colHeader"><s:text name="label.partReturn.RmaNo" /></th>
    </s:if>
     <s:elseif test="isProcessedAutomatically() && oemPartReplaced.partReturnConfiguration.rmaNumber != null">
      <th width="6%%" valign="middle" class="colHeader"><s:text name="label.partReturn.RmaNo" /></th>
     </s:elseif>
             <!-- CR NMHGSLMS-172 ends -->
     <s:if test="WPRA">
    ...
    <tr class="
      ...
      <input type="hidden"  size="3" name="partReplacedBeans[<s:property value="%{#partIterator.index}"/>].oemPartReplaced"  
      				value="<s:property value="partReplacedBeans[#partIterator.index].oemPartReplaced.id"/>"/>
      <td width="10%" style="padding-left:3px;"><s:property value="oemPartReplaced.brandItem.itemNumber"/></td>
     ...
     <!-- CR NMHGSLMS-172 ends -->
     <s:if test="WPRA">
            <td width="6%" style="padding-left:3px;"><s:property value="oemPartReplaced.getActivePartReturn().wpra.wpraNumber" /></td>
     </s:if>
      ...
      <td width="6%" style="padding-left:3px;"><s:property value="totalNoOfParts" /></td>
       <td width="7%" style="padding-left:3px;">
       <input type="hidden"  size="3" name="partReplacedBeans[<s:property value="%{#partIterator.index}"/>].toBeShipped"  
      				value="<s:property value="toBeShipped"/>"/>
       <input type="text" id="part_#taskIterator.index_ship" size="3" name="partReplacedBeans[<s:property value="%{#partIterator.index}"/>].ship"  
      ...
      <td width="7%" style="padding-left:3px;"><input type="text" id="part_#taskIterator.index_ship" size="3" name="partReplacedBeans[<s:property value="%{#partIterator.index}"/>].cannotShip"  
