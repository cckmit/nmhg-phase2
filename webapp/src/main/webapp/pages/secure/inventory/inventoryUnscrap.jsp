...
dojo.addOnLoad(function() {
	...
	dojo.connect(dojo.byId("unScrapFormSubmitButton"), "onclick", function(event) {
			confirmationDialog.show();
             dojo.stopEvent(event);
        });
	dojo.connect(dojo.byId('confirmationSubmitButton'), "onclick", function(){
		var form = document.unScrapform;
		form.submit();
	});
})
</script>
...
<u:body >
	<u:actionResults />
	<s:form action="unScrapInventory.action" method="POST" theme="twms"
		validate="true" id="unScrapform" name="unScrapform">
     ...
     <div dojoType="dijit.layout.LayoutContainer" style="width: 100%; height: 100%; overflow-x:hidden; overflow-y:scroll;" id="root" >
        ...
        <div id="dcap_pricing_title" class="section_header" style="margin-top:5px;margin-left:5px;margin-right:5px; ">
           ...
           <div dojoType="twms.widget.MultipleInventoryPicker"searchHandlerUrl="searchInventories_fleetUnScrap.action"
				useInnerHTML="false" selectionHandlerUrl="show_inventories_forUnScrap.action"
				selectedItemsContentPane="fleetInventoryUnScrap"></div>
			<div dojoType="dijit.layout.ContentPane" executeScripts="true"
				scriptSeparation="false" id="fleetInventoryUnScrap" >
        <table class="grid" cellspacing="0" cellpadding="0">
            ...
            <tr>
				...
				<td><s:property value="scrapDate" /></td>
				 <input type="hidden" name="scrapDate" 
			                     value="<s:property value="scrapDate"/>"/>
			</tr>
			<tr>
				<td class="labelStyle" nowrap="nowrap"><s:label value="%{getText('lable.unScrap.reasonForScrap')}" cssClass="labelStyle" /> :</td>
				<td><s:property value="%{scrapReason}" />
				<input type="hidden" name="scrapReason" 
			                     value="<s:property value="%{scrapReason}"/>"/>
				</td>				
			</tr>
			<tr>
				<td class="labelStyle" nowrap="nowrap"><s:label value="%{getText('label.scrap.unScrapDate')}" cssClass="labelStyle" /> :</td>
				<td><sd:datetimepicker name='unScrapDate' value='%{unScrapDate}' id='unScrapDate' /></td>
			</tr>
			<tr>
				...
				<div class="dialogContent" dojoType="dijit.layout.LayoutContainer"
					...
					<div dojoType="dijit.layout.ContentPane" layoutAlign="top" class="closableDialog" style="background: #F3FBFE;">
						<span class="TitleBar"  style="font-size: 9pt; display: inline;background: #F3FBFE; ">
						    <s:text name="label.unScrap.confirmation"></s:text>
						</span>
					</div>		
					
					<div dojoType="dijit.layout.ContentPane" layoutAlign="client" >
					    <br><s:text name="message.unScrap.confirmation" /> <br><br>
