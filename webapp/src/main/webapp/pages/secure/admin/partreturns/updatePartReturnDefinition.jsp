...
<u:body>
<u:actionResults/>
  <s:form name="baseForm" id="baseForm" action="update_part_return_definition" method="post" validate="true">  
  ...
  <table width="100%" class="grid" cellspacing="0" cellpadding="0" style="margin-left:3px;">
    <tr>
	  ...
	  <td>
		<div id="dealer">
			<sd:autocompleter
				...
				id='dealerAutoComplete' name='chosenDealer'
				keyValue="%{partReturnDefinition.forCriteria.dealerCriterion.dealer.id}"
				key="%{partReturnDefinition.forCriteria.dealerCriterion.dealer.id}"
				loadOnTextChange='true' loadMinimumCount='2'
				value='%{partReturnDefinition.forCriteria.dealerCriterion.dealer.name}'
				showDownArrow='false' autoComplete='false' />
		</div>
		<div id="dealerGroup">
		  <sd:autocompleter href='list_part_return_DealerGroupsInPartReturn.action' id='dealerGroupAutoComplete' name='dealerGroupName' value='%{partReturnDefinition.forCriteria.dealerCriterion.dealerGroup.name}' loadOnTextChange='true' showDownArrow='false' emptyOption='true' autoComplete='false' />
		</div>
	  </td>
      
      <td class="admin_data_table" width="15%"><s:text name="label.partReturnConfiguration.warrantyType" />:</td>
	  <td><s:select list="warrantyTypes" name="partReturnDefinition.forCriteria.warrantyType" listKey="type" listValue="%{getText(displayValue)}" headerKey="ALL" headerValue="All" value="%{currentWarrantyType}" theme="twms"/>
	  	</td>
	</tr>
	<tr>
	  <td class="admin_data_table"><s:text name="label.partReturnConfiguration.claimType" />:</td>
	  <td>
	  <s:select list="claimTypes" name="partReturnDefinition.forCriteria.claimType" listKey="type.toUpperCase()" listValue="%{getText(displayType)}" headerKey="ALL" headerValue="All"
	      value="%{currentClaimType}" theme="twms"/>
	  </td>
	  </td>
	  <td class="admin_data_table"><s:text name="label.partReturnConfiguration.productType" />:</td>
	  <td>
	    <sd:autocompleter href='list_part_return_Products.action' name='chosenProduct' loadOnTextChange='true' loadMinimumCount='2' value='%{partReturnDefinition.forCriteria.productType.itemGroupDescription}' showDownArrow='false' autoComplete='false' cssClass='admin_selections' />
	  </td>
	</tr>
	...
	<tr>
	  ...
	  <td>
	    <div id="item">
	      <sd:autocompleter href='list_part_return_ItemCriterions.action' id='itemAutoComplete' name='partCriterion' loadOnTextChange='true' loadMinimumCount='0' keyName='partReturnDefinition.itemCriterion.item.number' value='%{partReturnDefinition.itemCriterion.item.number}' showDownArrow='false' autoComplete='false' />
		</div>
		<div id="itemGroup">
		  <sd:autocompleter href='list_part_return_ItemGroupsForPartReturns.action' id='itemGroupAutoComplete' name='itemGroupName' keyName='partReturnDefinition.itemCriterion.itemGroup.name' value='%{partReturnDefinition.itemCriterion.itemGroup.name}' loadOnTextChange='true' loadMinimumCount='0' showDownArrow='false' autoComplete='false' />
	    </div>
	  </td>
	</tr>
	<tr>
		...
		<td>	        
	        <t:textarea id="shipping_instructions" rows="4" cols="40" maxLength="4000" value="%{partReturnDefinition.shippingInstructions}" name="partReturnDefinition.shippingInstructions"/>
	    </td>
	    <td class="labelStyle"> <s:text name="label.partReturnConfiguration.receiverInstructions"/>:</td>
	    <td>
	        <t:textarea id="receiver_instructions" rows="4" cols="40" maxLength="4000" value="%{partReturnDefinition.receiverInstructions}" name="partReturnDefinition.receiverInstructions"/>
	    </td>
	</tr>
	<tr>
		<td class="labelStyle"><s:text name="label.partReturnConfiguration.comments"/>:</td>
		<td colspan="3"> 
	        <t:textarea id="shipping_comments" rows="4" cols="40" maxLength="4000" value="%{partReturnDefinition.comments}" name="partReturnDefinition.comments"/>
	    </td>
	</tr>
</table>
...
<div class="policy_section_div" width="20%">
  ...
  <div align="center">
    ...
    <s:submit cssClass="buttonGeneric" value="%{getText('button.common.delete')}"  action="delete_part_return_definition" />
    <s:submit cssClass="buttonGeneric" value="%{getText('button.common.save')}"  action="update_part_return_definition"/>
      <s:if test="partReturnDefinition.status != 'INACTIVE' ">
    	<s:submit cssClass="buttonGeneric" id="inactiveButton" value="%{getText('button.partReturnDefinition.deActivate')}" onclick="updateStatus('INACTIVE', 'inactiveButton')"/>
    </s:if>
     <s:else>
    	<s:submit cssClass="buttonGeneric" id="activeButton" value="%{getText('button.partReturnDefinition.activate')}"  onclick="updateStatus('ACTIVE', 'activeButton')"/>
    </s:else>
  </div> 
  </s:form>
  ...
  <script type="text/javascript">
   ...
   <script type="text/javascript">
        function updateStatus(status, buttonId){
        	dojo.byId(buttonId).form.action = 'update_part_return_definition_status.action?status='+status;
            dojo.byId(buttonId).form.submit();
        }
   </script>
  <script type="text/javascript">
      dojo.addOnLoad(function() {
       ...
       <s:if test="partReturnDefinition!=null || partReturnDefinition.id!=null">
            <s:if test="!itemGroupSelected">
                dijit.byId("itemAutoComplete").setDisabled(true);
                dojo.html.hide(dojo.byId("toggleToItemGroup"));
            </s:if>
            <s:else>
                dijit.byId("itemGroupAutoComplete").setDisabled(true);
                dojo.html.hide(dojo.byId("toggleToItem"));
            </s:else>
 	 	</s:if>
      });
  </script>
