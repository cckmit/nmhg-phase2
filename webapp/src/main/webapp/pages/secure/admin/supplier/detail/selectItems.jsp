...
<div style="overflow: auto; width: 100%;">
	<table width="100%">
		<tr>
			...
			<th class="admin_table_heading"><s:text name='<s:text name="columnTitle.duePartsInspection.supplier_part_no"/>' /></th>
			<th class="admin_table_heading"><s:text name='<s:text name="label.sra.partSouce.supPartDescription"/>' /></th>
		</tr>
		<tr>
		...
		<s:iterator value="includeItems">
			<tr>
				<td align="center" class="admin_selections">&nbsp;</td>
		...
		<s:iterator value="items" status="iter">
			<s:if test="%{includeItems.contains(top)}"></s:if>
			<s:else>
				<tr>
					<td align="center"><s:if test="%{prevItems.contains(top)}">
							<input type="checkbox" name="available"
								value="{'id':'<s:property value="top.id"/>',
                           'number':'<s:property value="top.number"/>',
                           'description':'<s:property value="top.description"/>'}" />
						</s:if> <s:else>
							<input type="checkbox" value="<s:property value="code"/>" disabled />
						</s:else></td>
					<td class="admin_selections"><s:property value="number" /></td>
					<td class="admin_selections"><s:property value="description" /></td>
				</tr>
	...
	<div>
		<center>
			<s:iterator value="pageNoList" status="pageCounter">
				...
				<s:else>
					<span id="page_<s:property value="%{#pageCounter.index}"/>" style="cursor: pointer; text-decoration: underline"> <a
						href="javascript:searchItems('search_items_for_contract.action',<s:property value="%{intValue()-1}" /> , 'searchResults') "> <s:property
								value="%{intValue()}" />
					</a>
