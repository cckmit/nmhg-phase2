			...
			<s:iterator	value="campaign.hussPartsToReplace"
								...
								<s:iterator value="campaign.hussPartsToReplace[#mainIndex.index].removedParts" status="subIndex">								
								   ...
								   <script type="text/javascript">
							            dojo.addOnLoad(function(){
							            	...
							            	dojo.connect(dijit.byId("replacedPartNumber_"+inIndex+"_"+subIndex),"onChange",function(){
							            	if (remPartNo!='')
							            		dijit.byId("replacedPartNumber_"+inIndex+"_"+subIndex).setDisplayedValue(remPartNo);							            									            									            	
							            	});
							            });
						            </script>
								...
								<s:hidden
									...
									<script type="text/javascript">
							            dojo.addOnLoad(function(){
							            	...
							            	var instPartId = '<s:property value="campaign.hussPartsToReplace[#mainIndex.index].installedParts[#subIndex.index].item.id"/>';
							            								            								            	
							            	var inIndex='<s:property value="%{#mainIndex.index}"/>';
											var subIndex='<s:property value="%{#subIndex.index}"/>';							            								            	
							            	dojo.connect(dijit.byId("installedHussmanPartNumber_"+inIndex+"_"+subIndex),"onChange",function(){
							            	if (instPartNo!='')
							            		dijit.byId("installedHussmanPartNumber_"+inIndex+"_"+subIndex).setDisplayedValue(instPartNo);							            									            									            	
							            	});
							            });
						            </script>
				...
				<table width="100%" >
					<tbody id='addRepeatBody_NonHussInstalled_<s:property value="%{#mainIndex.index}" />'>
						<tr class="title" >
							<td width="50%" nowrap="nowrap"><s:text name="label.newClaim.nonHussmanPartsInstalled" /></td>
							<td width="20%"/>
							<td width="20%"/>
							<td width="10%">
						...
						<tr class="row_head">
							<td width="50%" align="center" class="partReplacedClass">
							<s:text name="label.common.description" />								
							</td>
							<td width="20%" align="center" class="partReplacedClass">
								<s:text name="label.common.quantity" />
						...
						<s:if test="campaign.hussPartsToReplace[#mainIndex.index].nonOEMpartsToReplace != null">
							<s:iterator	value="campaign.hussPartsToReplace[#mainIndex.index].nonOEMpartsToReplace"
													status="subIndex">
								<s:hidden
                                            name="campaign.hussPartsToReplace[%{#mainIndex.index}].nonOEMpartsToReplace[%{#subIndex.index}]"
                                            id="nonHussInstalledPart_%{#mainIndex.index}_%{#subIndex.index}"
                                            value="%{campaign.hussPartsToReplace[#mainIndex.index].nonOEMpartsToReplace[#subIndex.index].id}"/>
								<tr id="NonHussmannInstalledRow_<s:property value='%{#mainIndex.index}' />_<s:property value='%{#subIndex.index}' />" subNonHussInstallRowIndex="<s:property value='%{#subIndex.index}'/>">
									<td width="50%" align="center" class="partReplacedClass">
										...
										name="campaign.hussPartsToReplace[<s:property value='%{#mainIndex.index}' />].nonOEMpartsToReplace[<s:property value='%{#subIndex.index}' />].i18nNonOemPartsDescription[0].description"
										value="<s:property value='campaign.hussPartsToReplace[#mainIndex.index].nonOEMpartsToReplace[#subIndex.index].i18nNonOemPartsDescription[0].description'/>" />
										<s:hidden  name="campaign.hussPartsToReplace[%{#mainIndex.index}].nonOEMpartsToReplace[%{#subIndex.index}].i18nNonOemPartsDescription[0].locale"
					 						value="%{defaultLocale}"/>
									</td>
									<td width="20%" align="center" class="partReplacedClass">
											<s:textfield id="replacedQuantity_%{#mainIndex.index}_%{#subIndex.index}" size="3"
											name="campaign.hussPartsToReplace[%{#mainIndex.index}].nonOEMpartsToReplace[%{#subIndex.index}].noOfUnits" />
									</td>
									<td width="20%" align="center" class="partReplacedClass">
										<t:money id='installedNonHussmanPrice_%{#mainIndex.index}_%{#subIndex.index}' size="3"
												name="campaign.hussPartsToReplace[%{#mainIndex.index}].nonOEMpartsToReplace[%{#subIndex.index}].pricePerUnit"
												value="%{campaign.hussPartsToReplace[#mainIndex.index].nonOEMpartsToReplace[#subIndex.index].pricePerUnit}" 
												defaultSymbol="%{'$'}" />
									</td>
									<td width="10%" align="center" class="partReplacedClass">
