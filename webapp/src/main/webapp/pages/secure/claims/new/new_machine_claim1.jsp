...
<s:form method="post" theme="twms" validate="true" id="form"
        name="saveMachineClaimDraft" action="saveMachineClaimDraft.action">
...
<s:hidden name="dealerNumberSelected" id="isDealerNumber"/>
  ...
  <tr><td>&nbsp;</td></tr>
        ...
        <tr>
            <td width="26%">
                <s:if test="isLoggedInUserADealer() || claim.itemReference.referredInventoryItem.serialNumber != null">
            ...
            <td style="text-transform:uppercase; padding-left:5px;" width="35%">
                <s:if test="claim == null || claim.id == null">
                      ...
                      <s:if test="isLoggedInUserADealer()">
                        ...
                        <script type="text/javascript">
					        dojo.addOnLoad(function() {
					            dojo.byId("dealerIdForSearch").value='<s:property value="getLoggedInUser().getCurrentlyActiveOrganization().id"/>';
							});
    					</script>                        
                    </s:if>
                    <s:else>
					...
					test="claim.itemReference.referredInventoryItem.serialNumber != null">
                      <s:property value="getClaimDetail().getItemReference().getReferredInventoryItem().getCurrentOwner().name"/>
                        <s:hidden name="dealer" value="%{getClaimDetail().getItemReference().getReferredInventoryItem().getCurrentOwner()}"/>
                        <s:hidden name="claim.forDealer" value="%{getClaimDetail().getItemReference().getReferredInventoryItem().getOwner().id}" id="dealer"/>
                        <script type="text/javascript">
					        dojo.addOnLoad(function() {
					            dojo.byId("dealerIdForSearch").value='<s:property value="getClaimDetail().getItemReference().getReferredInventoryItem().getOwner().id"/>';
                                });
    					</script>     
                    </s:if>
                    <s:else>
