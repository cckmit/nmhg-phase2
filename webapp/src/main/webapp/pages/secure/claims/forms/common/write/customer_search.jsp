...
<table border="0" align="center" cellpadding="0" cellspacing="0" id="individual_Search" class="grid borderForTable">
	...
	<tbody id="individual_customer_list">			
		<s:iterator value="matchingCustomerList" id="customer" status = "custStatus">
		<s:iterator value="addresses" status="addressStatus">
			<script>
			    	addressDetails_<s:property value="#custStatus.index"/>_<s:property value="#addressStatus.index"/> = new Array("<s:property value="%{#customer.companyName}"/>","<s:property value="city"/>","<s:property value="state"/>","<s:property value="zipCode"/>","<s:property value="country"/>","<s:property value="id"/>");
			 </script>
		<tr>
			...
			<td width="10%" nowrap="nowrap"  >
				  <s:if test="dealerOrganization.id == loggedInUser.belongsToOrganization.id">
					 <div align="center"> <a class="Datalink" 
						href="javascript:showAddressDetails(addressDetails_<s:property value="#custStatus.index"/>_<s:property value="#addressStatus.index"/>)" > <s:text name="button.common.addLabel"/> </a> |  <a class="Datalink" 
					    href="javascript:showCustomerUpdate(<s:property value="%{#customer.id}"/>,<s:property value="id"/>)" > <s:text name="button.common.edit"/> </a> 
					 </div>
				  </s:if>
				  <s:else>
				    <div align="center"> <a class="Datalink" 
...
<table border="0" align="center" cellpadding="0" cellspacing="0" id="individual_Search" class="grid borderForTable">
	...
	<tbody id="individual_customer_list">	
	<%-- [<s:property value = "matchingCustomerList.size()"/>] --%>
	<%-- <s:property value = "matchingNAList.size()"/> --%>
