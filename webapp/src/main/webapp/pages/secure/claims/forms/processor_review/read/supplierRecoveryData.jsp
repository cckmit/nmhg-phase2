...
<div class="section_header" >
	<s:text name="title.common.supplier.recoveryInfo"></s:text>
	    	...
	    	<table  width="96%" class="grid borderForTable" cellspacing="0" cellpadding="0" align="center">
	    		<thead>
	    		...
	    		<tbody>
	    			<tr>
	    				...
	    				<td width="25%">
	    				<sd:autocompleter name='selectedContract' href='list_claim_contracts.action?' key='%{task.claim.serviceInformation.contract.id}' value='%{task.claim.serviceInformation.contract.name}' loadMinimumCount='0' listenTopics='/causalPart/changed/queryParams' indicator='contract_indicator' id='contractId' />
						 <img style="display: none;" id="contract_indicator" class="indicator"
                    		 src="image/indicator.gif" alt="Loading..."/>
					</td>

					<td width="25%">
					 <s:a cssStyle="cursor:pointer; color:#0000FF;text-decoration:underline">
 ...
 dojo.addOnLoad(function() {
     ...
     dojo.publish("/causalPart/changed/queryParams", [{
         params: {
             ...
             "causalPart": <s:property value='task.claim.serviceInformation.causalPart.alternateNumber'/>
         },
         makeLocal: true
     }]);
		...
		twms.ajax.fireHtmlRequest("get_contractDetails.action", {
                    contractId: dijit.byId("contractId").getValue()
							...
							for(i =0 ; i < count ; i++)
							{
								...
								{
                                    if(dojo.byId("oemPart_toBeReturned_installed")){
                                        dojo.byId("oemPart_toBeReturned_installed").checked = true;
                                    }else{
                                        if(dojo.byId("oemPart_" + i + "_toBeReturned")){
                                            dojo.byId("oemPart_" + i + "_toBeReturned").checked=true
                                        }                                        
                                    }
                                }
							}
