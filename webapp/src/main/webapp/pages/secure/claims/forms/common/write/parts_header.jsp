...
<table class="form" style="border-top:1px solid #EFEBF7; border-bottom:none; border-left:none; border-right:none;" cellpadding="0" cellspacing="0">
    <tbody>
                ...
                <label for="type"id="brandTypeLabel" class="labelStyle"><s:text name="label.common.claimBrand"/>:</label>
            </td>
          
             <td style="padding-top:5px;" valign="top">  
          <s:if test= "claim != null && claim.itemReference != null && claim.itemReference.referredInventoryItem !=null ">

            <sd:autocompleter id='brandType' listenTopics='dealer_brand' name='claim.brand' value= "%{claim.itemReference.referredInventoryItem.brandType}" readonly="true"/>
            </s:if>
            <s:else>
           ...
           <script type="text/javascript">
                        ...
                        dojo.publish("/brand/changed", [{
                            params: {
                                "claimBrand" : '<s:property value="%{claim.itemReference.referredInventoryItem.brandType}" />'
                            }
                        }]);
                        <s:if test= "claim == null || claim.id == null">
                           ...
                           if(dijit.byId("eqpSerialNumber"))
                           {
                            ...
                            dojo.connect(dijit.byId('brandType'), "onChange", function(newValue) {
                         ...
                         <s:else>
                         dijit.byId('brandType').setDisabled(true);
                         </s:else>
                        })
                    </script>
            </td>
			</tr>
    	<jsp:include flush="true" page="partAndSerialNumber.jsp" />    	
		...
		<s:else>
			<s:if test="(!claim.partItemReference.serialized && !claim.partInstalled)">				
				<tr>
					...
					<td nowrap="nowrap"><sd:datetimepicker name='claim.purchaseDate' value='%{claim.purchaseDate}' id='purchaseDate' /></td>
				</tr>
			</s:if>
        ...
        <tr>        
            ...
            <s:else>
      	         <td>
                    ...
                    <sd:autocompleter id='eqpSerialNumber' href='list_claim_sl_nos.action?selectedBusinessUnit=%{selectedBusinessUnit}&claimType=%{claimType}' loadOnTextChange='true' loadMinimumCount='2' showDownArrow='false' name='selectedItems' value='%{claim.itemReference.referredInventoryItem.serialNumber}' notifyTopics='invItemChanged/setSerialNumber' listenTopics="/dealer/modified,/brand/changed"/>
                    <script type="text/javascript">
                        dojo.addOnLoad(function() {       
...
jQuery(document).ready(function(){
    ...
    function showSerialNumberSection(){      
        ...
        function hideBrandType(){
    	 if(dijit.byId("brandType")){
    	 
    	        dojo.html.hide(dijit.byId("brandType").domNode);
    	        
    	        }
    	        if(dojo.byId("brandTypeLabel")){
    	        dojo.html.hide(dojo.byId("brandTypeLabel"));
    	        }
    }
    function showBrandType(){      
       dojo.html.show(dijit.byId("brandType").domNode);
       dojo.html.show(dojo.byId("brandTypeLabel"));
    }
    function enablePartNumber() { 
    var brand = '<s:property value="claim.brand"/>';
    	 if((dijit.byId("brandType").value!=null && dijit.byId("brandType").value!="") || brand)
					     	 { 	  
        dijit.byId("brandPartNumber").setDisabled(false);
        }
        else
        {
         dijit.byId("brandPartNumber").setDisabled(true);
        }
        dojo.byId("partSerializedId").value=false;
  ...
  function showEqSerialNumber() {	 
	  ...
	  if(dijit.byId("eqpSerialNumber")&& dijit.byId("brandType").getValue()!=null && dijit.byId("brandType").getValue()!=""){
  	    dijit.byId("eqpSerialNumber").setDisabled(false); 
	  }
	  if(dijit.byId("productModel")){  
	  dojo.html.hide(dijit.byId("productModel").domNode); 
