...
<table cellspacing="0" cellpadding="0" class="grid borderForTable" align="center" width="100%">
    <thead>        
        <tr class="row_head">
            <authz:ifProcessor>
                <s:if test="claim.forMultipleItems && processorReview">
    ...
    <authz:ifProcessor>
        ...
        <td >
        	<s:if test="claim.itemReference.isSerialized()">
				<s:if test="itemReference.referredInventoryItem!=null">
    	    	    ...
    	    	    <authz:else>
    	    	    <s:if test="itemReference.referredInventoryItem.d.isActive()">
    	    	    <s:set name="itemReferred" value="itemReference.referredInventoryItem.id"/>
			         ...
			         <s:else>
			         	<span><s:property value="itemReference.referredInventoryItem.serialNumber" /></span>
			         </s:else>
        		    </authz:else>
			    </s:if>
			  </s:if>  
        ...
        <td >
			...
			<s:else>			
				<s:property value="getProduct(itemReference.model).groupCode" />
			</s:else>
        </td>
        <td>        	
			<s:if test="itemReference.referredInventoryItem!=null">
        ...
        <td >		
            <s:if test="itemReference.serialized">	
           		...
           		<s:else>			
				<s:property value="getProduct(itemReference.model).itemGroupDescription" />
			</s:else>		
        </td>  
        <td>
        	<s:hidden name="claim.claimedItems[%{#claimedItems.index}].hoursInService"
	        			value="%{hoursInService}" />
	        <s:property value="hoursInService" />        		
        </td> 
        <authz:ifUserNotInRole roles="supplier"> 
    ...
    <table width="95%" border="0" cellpadding="2" cellspacing="2">
		   <tr>
		       <td align="left"  style="padding:3px">
				   	<span style="color: green;">
				   		<s:text name="warrantyDateDisclaimer" /> 
				   	</span>
			   </td>
		   </tr>	
	   </table>
