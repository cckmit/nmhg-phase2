 ...
 <s:if test="dealerEligibleToFillSmrClaim || claim.serviceManagerRequest">
   ...
   <tr>
     ...
     <s:if test="(isLoggedInUserAnInternalUser() || context.equals('ClaimSearches')) && displayNCRandBT30DayNCROnClaimPage()" >
		...
		<s:if test="displayNCRandBT30DayNCROnClaimPage() && claim.state.state.equals('draft') && 
					isLoggedInUserADealer() && getAllowed30DayNcrClasses().isEmpty()">
			<!-- Current logged in user is a dealer who is not allowed 30 Day NCR machine claim.
			     In this case, when dealer is filing a machine claim, 30 Day NCR claim label and 
			     value will not be shown, as in page 1, this user does not have option to select 
			     30 Day NCR class. This is as per SLMSPROD-970 requirements. -->
		</s:if>
		<s:elseif test="displayNCRandBT30DayNCROnClaimPage() && isLoggedInUserADealer() && 
					!claim.state.state.equals('draft') && claim.ncrWith30Days">
			<!-- This section is to test whether claim is not a draft (i.e it is already filed). 
			     If claim was filed as a 30 Day NCR claim (the dealer may not have privilege to 
			     file 30 Day NCR today, but we can show earlier submitted 30 Day NCR claim
			     by this dealer), then claim will carry 30 Day NCR UI element.
			     
			     The logic is that if claim can be opened/seen by the dealer, then show 
			     the UI elements for 30 Day NCR, if the claim by submitted as 30 Day NCR,
		...
		<s:elseif test="displayNCRandBT30DayNCROnClaimPage() && isLoggedInUserADealer() && 
					!claim.state.state.equals('draft') && !claim.ncrWith30Days">
			<!-- This condition is opposite of the above - Don't show -->
		</s:elseif>
		<s:elseif test="displayNCRandBT30DayNCROnClaimPage()">
		    <!-- In this case, either an eligible dealer user is there, or a user who 
		         is authorized to open/search for this claim. Hence, we are showing whether this
		         claim is 30 Day NCR or not -->			
			<td class="labelStyle" style="padding-bottom: 5px;" nowrap="nowrap"><s:text
					name="label.claim.ncrWith30Days" />:</td>
         ...
         <tr>
          <td class="labelStyle"  style="padding-bottom:5px;" nowrap="nowrap"><s:text name="label.newClaim.commercialPolicy"/>:</td>
	          ...
	          <s:if test="claim.invoiceNumber != null">
    	   	...
    	   	<authz:ifUserInRole roles="processor">
   	<s:if test="policyCodes.size()>0 && #policyCodeEditable == true && !amerClaimAcceptedEarlier(claim)">
          <tr>
          <td width="22%">
            <label for="policyCode"  class="labelStyle"><s:text name="label.viewClaim.policyCode"/>:</label>
        </td>
...
<s:if test="claim.forDealer.name!=null">
  <jsp:include page="dealerInfoPage.jsp"/>
</s:if>
