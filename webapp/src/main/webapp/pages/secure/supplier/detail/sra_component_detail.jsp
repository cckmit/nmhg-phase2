...
dojo.addOnLoad(function() {
	if (typeof(dijit.byId("aceptedAmountNonEditable")) != "undefined")
		{
		dijit.byId("aceptedAmountNonEditable").set("readOnly", true);
		}
    });
...
function computeTotalValue(totalCount,field){
    ...
    document.getElementById("aceptedAmount").value=totalvalue.toFixed(2);
    document.getElementsByName('recoveryClaim.acceptedAmount')[1].value=totalvalue.toFixed(2);
     
}

function showWrngMsg(){
	var supplier=<s:property value="isLoggedInUserASupplier()" />;
	if(supplier){
	if(document.getElementById("totalValue").innerHTML!=document.getElementById("aceptedAmount").value)
		  dojo.html.show(accpAmtWrngMsg);
	else
		dojo.html.hide(accpAmtWrngMsg);
	}
}
</script>
	                   ...
	                   <td  valign="middle">
		                   <authz:ifUserInRole roles="recoveryProcessor">                   
			                   <s:if test="recoveryClaim.recoveryClaimState.state =='Debited and Closed' || 
									recoveryClaim.recoveryClaimState.state == 'Awaiting Supplier Response' ||
									recoveryClaim.recoveryClaimState.state == 'Shipment From WareHouse' ||
									recoveryClaim.recoveryClaimState.state == 'Accepted' ||
									recoveryClaim.recoveryClaimState.state == 'Ready for Debit' ||
									recoveryClaim.recoveryClaimState.state == 'Closed Recovered' ||
									...
									recoveryClaim.recoveryClaimState.state == 'Requested' || 
									recoveryClaim.recoveryClaimState.state == 'Wait for Debit'">
			                     <s:property value="%{#costLineItem.recoveredCost}"/>
			                   </s:if>
			                   <s:else>
			                   <t:money
			                       id="Actual_Value_%{#lineItemStatus.index}"
			                       name="recoveryClaim.costLineItems[%{#lineItemStatus.index}].recoveredCost"
			                       defaultSymbol="$" value="%{#costLineItem.recoveredCost}" size="7"
			                       maxlength="10" cssStyle="margin:-1px"
			                       onchange="computeTotalValue('%{#attr['totalCount']}',this)" />
			                   </s:else>
		                    </authz:ifUserInRole>
		                    
		                    <authz:ifUserNotInRole roles="recoveryProcessor">
		                   		 <s:property value="%{#costLineItem.recoveredCost}"/>
		                    </authz:ifUserNotInRole>
	                   </td>
                   </authz:ifUserNotInRole>
           ...
           <tr>
              ...
              <tr>             
               ...
               <td width="15%" >
                ...
                <s:if test="(isLoggedInUserASupplier() && !buConfigAMER) || getLoggedInUser().isInternalUser()">
               <td width="30%"><s:property
                   value="%{getTotalCostForSection('costAfterApplyingContract',#attr['supplierCurrency'])}"/>
               </td>
               </s:if>
                <td>
                <s:if test="recoveryClaim.recoveryClaimState.state =='Debited and Closed' || 
									...
									recoveryClaim.recoveryClaimState.state == 'Wait for Debit'">
			                    <t:money
			                       id="aceptedAmountNonEditable"
			                       name="recoveryClaim.acceptedAmount"
			                       defaultSymbol="$" value="%{getAcceptedCostForSection('recoveredCost',#attr['supplierCurrency'])}" size="7"
			                       maxlength="10" cssStyle="margin:-1px"
			                       />
			                   </s:if>
			                   <s:else>
               			  <t:money
			                       ...
			                       name="recoveryClaim.acceptedAmount"
			                       defaultSymbol="$" value="%{getAcceptedCostForSection('recoveredCost',#attr['supplierCurrency'])}" size="7"
			                       maxlength="10" cssStyle="margin:-1px" onchange="showWrngMsg()"/>
			                       
			                      <span id="accpAmtWrngMsg" style="display:none;" tabindex="0" >
								<img src="image/warning.gif" width="16" height="15" />
								</span>
								<span dojoType="dijit.Tooltip" connectId="accpAmtWrngMsg" >
								
								<s:text name="message.sra.acceptedAmount" >
								<s:param><s:property value="recoveryClaim.contract.supplier.preferredCurrency.currencyCode"/></s:param></s:text></span>	
			                        </s:else>
			                       </td>
           </tr>
       </table>
