...
public class ListPendingReviewCampaigns extends SummaryTableAction {
	...
	protected PageResult<?> getBody() {
		...
		String businessUnitName=getCurrentBusinessUnit().getName();
        return campaignAssignmentService
		.findAllCampaignNotificationRequestsByStatus(createNotificationCriteria(
				dealer, new PageSpecification(page - 1, pageSize)),FieldModUpdateStatus.SUBMITTED,isLoggedInUserADealer(),businessUnitName);
	}
	...
	private CampaignAssignmentCriteria createNotificationCriteria(
			ServiceProvider dealership, PageSpecification pageSpecification) {
		...
		listCriteria.setDealer(dealership);
		return listCriteria;
	}
