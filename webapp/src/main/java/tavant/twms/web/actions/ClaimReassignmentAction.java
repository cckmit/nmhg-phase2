 ...
 */
public class ClaimReassignmentAction extends I18nActionSupport {
    ...
    public String reassignment() {
        ...
        if (StringUtils.hasText(getPassword())) {
            claims = claimRepository.findClaimToReassign();
        } else {
        ...
        for (Claim claim : claims) {
            ...
            List<String> eligibleProcessors = assignmentRuleExecutor
                    .fetchEligibleProcessorsUsingAssignmentRules(claim);
            if (eligibleProcessors == null || eligibleProcessors.size() == 0) {
                User defaulfUser = orgService.findDefaultUserBelongingToRoleForSelectedBU(claim.getBusinessUnitInfo().getName(), Role.PROCESSOR);
                if (defaulfUser != null && !"".equalsIgnoreCase(defaulfUser.getName()))
                    setCorrectAsignee(defaulfUser.getName());
                else
                    setCorrectAsignee("processor");
            } else {
                setCorrectAsignee(getUserWithLeastLoad(eligibleProcessors));
            }
            List<TaskInstance> openTasks = workListDao.getAllOpenTasksForClaim(claim);
            for (TaskInstance openTask : openTasks) {
                if (openTask.getName().equals("Processor Review") ||
                        openTask.getName().equals("On Hold For Part Return") ||
                        (StringUtils.hasText(task) && openTask.getName().equals(task))) {
