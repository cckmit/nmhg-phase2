...
public class ForwardedClaimsAutoReplyAction extends Action {
	...
	throws Exception {
		...
		if(claim != null)
		{
			int dueDays = configParamService.getLongValue(ConfigName.DAYS_FOR_AUTO_REPLY.getName()).intValue();
			SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
			Date date = sdf.parse(Clock.today().plusDays(dueDays+10).toString(DATE_FORMAT));
