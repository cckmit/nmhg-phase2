...
public class SingleTokenJoinTest extends ProcessDeployableTestCase {
    ProcessInstance processInstance;

    @Override
    protected void setUpInTxnRollbackOnFailure() throws Exception {
        super.setUpInTxnRollbackOnFailure();
        ProcessDefinition def = deployProcess("single-token-join-process.xml");
        processInstance = new ProcessInstance(def);
    }

    @SuppressWarnings("unchecked")
    public void testByEndingSale() {
    ...
    @SuppressWarnings("unchecked")
    public void testByEndingMonitor() {
        processInstance.signal();
        assertFalse(processInstance.hasEnded());
        Token rootToken = processInstance.getRootToken();
        assertEquals("SingleTokenFork", rootToken.getNode().getName());
        ...
        monitorSaleTask.setVariable("sale", "Done");
        monitorSaleTask.end();
