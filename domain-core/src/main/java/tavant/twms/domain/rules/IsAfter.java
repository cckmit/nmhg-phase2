...
public class IsAfter implements BinaryPredicate {
    ...
    public IsAfter(Value oneValue, Value anotherValue) {
        ...
        Assert.isTrue(oneValue.getType().equals(Type.DATE)," is after check applies ONLY to date types");        
        this.lhs = oneValue;
    ...
    public String getDomainTerm() {
        return "label.operators.isAfter";
    }
    
    public Predicate getInverse() {
		return new IsOnOrBefore(lhs,rhs);
	}
