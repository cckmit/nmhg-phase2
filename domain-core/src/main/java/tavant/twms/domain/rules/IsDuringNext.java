...
public class IsDuringNext extends AbstractDateDurationPredicate {

    public IsDuringNext(DomainSpecificVariable oneValue, Constant anotherValue,
                      int durationType) {
        super(oneValue, anotherValue, durationType);
    }

    public IsDuringNext(DomainSpecificVariable oneValue,
    ...
    public void validate(ValidationContext validationContext) {
        ...
        if (DateType.DurationType.DAY.getType() == getDurationType()) {
            validationContext.addError("during next check can take " +
                    "only calendar week or month as rhs");
        }
    }

    public String getDomainTerm() {
        return "label.operators.isDuringTheNext";
    }
    ...
    public BinaryPredicate getInverse() {
		return new IsNotDuringNext(lhs, dateToCompare, rhs, durationType);
	}
}
