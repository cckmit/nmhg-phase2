...
/**
 * Hibernate NoCurrencyMoneyUserType class for persisting objects of Money class, currently
 * only the amount portion is being persisted the currency is assumed to be
 * dollars. We would however use only the amount.
 * 
 * 
 */
public class NoCurrencyMoneyUserType implements CompositeUserType {
    ...
    public boolean equals(Object first, Object second) throws HibernateException {
        ...
        if (first == null || second == null)
            return false;
        if (((Money)first).breachEncapsulationOfAmount() == null || ((Money)second).breachEncapsulationOfAmount() == null)
            return false;
        return ((Money)first).breachEncapsulationOfAmount().equals(((Money)second).breachEncapsulationOfAmount());
//        return (second);
    }
...
//        Currency cur = (Currency) Hibernate.CURRENCY.nullSafeGet( rs, names[1] );
        ...
        return Money.valueOf(amt, Currency.getInstance("USD"));
    }
