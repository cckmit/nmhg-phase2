...
||
*/
PROCEDURE process_claim_audit_xml_str;
...
||
*/
PROCEDURE process_claim_audit_xml_str AS
  CURSOR claim_audits
  is
...
AS
  ...
  SELECT row_num, id, xml_src_data FROM (
	SELECT ROWNUM row_num , id, xml_src_data
		FROM claim_audit_temp) t 
	WHERE t.row_num >= p_in_id_start_range AND t.row_num <= p_in_id_end_range;
