...
BEGIN
  ...
  IF V_PK_COLUMN_COUNT = 1 THEN
      ...
      v_sql_stmt:= 'CREATE TABLE ' || p_in_stg_table_name || ' AS SELECT stg.*,' || v_pk_column_name ||' AS OLD_43_ID,''Y'' AS MIGRATE_FLAG  FROM ' || p_in_60_schema_owner ||'.'|| p_in_table_name || ' stg ' || ' WHERE 1=2';

  ELSE

      v_sql_stmt:= 'CREATE TABLE ' || p_in_stg_table_name || ' AS SELECT stg.*,''Y'' AS MIGRATE_FLAG  FROM ' || p_in_60_schema_owner ||'.' || p_in_table_name || ' stg ' || ' WHERE 1=2';
