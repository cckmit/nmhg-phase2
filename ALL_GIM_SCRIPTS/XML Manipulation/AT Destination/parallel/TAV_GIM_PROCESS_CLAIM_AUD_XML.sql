...
TYPE t_claim_audit_job_rec IS RECORD(v_row_number NUMBER,
                                    v_id NUMBER,
                                    v_xml_data CLOB);
...
cursor CLAIM_AUDITS(V_ID_START_RANGE number, V_ID_END_RANGE number)
IS
   select Y.*
   from
    (
        ...
        from
        (
              select ca.*, c.claim_number from claim_audit ca, claim c
              Where C.Id = Ca.For_Claim
              -- and c.claim_number = 'HUS-20000022'
              and length(CA.id) > 13
              AND cA.ID < 110000000000000
  ...
  where Y.ROW_NUM >= P_IN_ID_START_RANGE
  AND Y.row_num   <= p_in_id_end_range;
...
begin
  ...
  BEGIN
	  FOR EACH_CLAIM_AUDIT IN claim_audits(p_in_id_start_range,P_IN_ID_END_RANGE)
	  loop
      ...
      str := tav_gim_xml_utilities.read_line(each_claim_audit.prev_claim_snapshot_string, has_more_data);
   ...
   /*
		Insert
		INTO claim_audit_backup VALUES
		  (
			each_claim_audit.id,
     ...
     */
      ...
      set ca.prev_claim_snapshot_string = new_audit_xml, d_internal_comments = d_internal_comments || ' | XML_UPDATED',d_updated_on = sysdate
...
AS
  ...
  CURSOR CUR_CLAIM_AUDIT_DET(V_ID_START_RANGE NUMBER, V_ID_END_RANGE NUMBER) IS
  SELECT ROW_NUM, ID, XML_SRC_DATA
  FROM
      (SELECT ROWNUM ROW_NUM, ID, XML_SRC_DATA
       FROM
            (
              SELECT a.id as ID,a.prev_claim_snapshot_string as XML_SRC_DATA
              FROM claim_audit a,claim b
              WHERE b.id = a.for_claim
              AND LENGTH(A.ID) > 13
              AND A.ID < 110000000000000
...
BEGIN
  ...
  OPEN cur_claim_audit_det(p_in_id_start_range,p_in_id_end_range);
  LOOP
    v_stmt_number := 60;

    FETCH cur_claim_audit_det BULK COLLECT INTO v_claim_audit_job_rec_tab LIMIT 100;
     EXIT WHEN cur_claim_audit_det%NOTFOUND;
